...
REPO = "/var/tmp/build/dabsrc/src/main/resources/assets/dabywitherstormmod/textures/entity"
...
def process_white(fname, out_name):
    """Phase 5 keeps WHITE teeth: same emissive mask, no turquoise tint."""
    p = os.path.join(REPO, fname)
    if not os.path.exists(p):
        return None
    ...
    for (x0, y0, x1, y1) in HEAD_BOXES:
        sub = pale_mask(a[y0:y1, x0:x1])
        rows = [r for r in range(sub.shape[0]) if sub[r].any()]
        if not rows:
            continue
        bands, run = [], [rows[0]]
        for r in rows[1:]:
            if r == run[-1] + 1:
                ...
                bands.append(run); run = [r]
        bands.append(run)
        mouth = bands[-1][-2:]
        for r in mouth:
            mask[y0 + r, x0:x1] |= sub[r]
    em = np.zeros_like(a)
    em[mask] = [255, 255, 255, 255]
    outp = os.path.join(REPO, out_name)
    Image.fromarray(em.astype(np.uint8), "RGBA").save(outp)
    return int(mask.sum()), outp
...
if __name__ == "__main__":
    ...
    # phase 5 = white teeth (user: "phase 5 doesn't have turquoise, it remains white")
    for src, out in (("wither_storm.png", "wither_storm_p5_e.png"),
                     ("wither_storm_og.png", "wither_storm_og_p5_e.png")):
        r = process_white(src, out)
        if r:
            print(f"  {out}: {r[0]} white teeth px")
