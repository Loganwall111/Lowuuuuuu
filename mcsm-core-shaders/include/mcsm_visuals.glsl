...
// 7000.0.11-M MERGED VOID ADDITIONS - Sift Cosmos visual helpers
vec3 mcsm_hsv2rgb(vec3 c) {
    vec4 K = vec4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
    vec3 p = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}
vec3 hsv2rgb(vec3 c) {
    return mcsm_hsv2rgb(c);
}
vec3 calculateGodRays(vec3 worldPos, vec3 viewDir, float depth) {
    float t = witherstorm_GameTime;
    if (t == 0.0) t = 100.0;
    ...
    float colorShift = sin(t * 0.01 + worldPos.x * 0.01) * 0.5 + 0.5;
    vec3 rayColor = mix(mix(rayColor1, rayColor2, colorShift), rayColor3, sin(t * 0.007) * 0.5 + 0.5);
    float wave = sin(worldPos.x * 0.02 + t * 0.02) * sin(worldPos.z * 0.02 + t * 0.015) * 0.3 + 0.7;
...
vec3 iridescentWater(vec3 worldPos, vec3 viewDir, vec3 normal, float time) {
    ...
    vec3 base = mcsm_hsv2rgb(vec3(hue, 0.8, 0.9));
