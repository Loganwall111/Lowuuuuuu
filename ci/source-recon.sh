...
java -Xmx3g -jar "$VF" "$DL/net" "$DL/out" > "$DL/vineflower.log" 2>&1 \
  || die "vineflower run failed" "$DL/vineflower.log"

mkdir -p src-recon
cp -r "$DL/out/net" src-recon/ 2>/dev/null || die "no decompiled tree produced" "$DL/vineflower.log"
N_JAVA=$(find src-recon -name '*.java' | wc -l)
echo "[recon] stage 3 ok: $N_JAVA java files recovered"
[ "$N_JAVA" -gt 100 ] || die "only $N_JAVA java files recovered" "$DL/vineflower.log"
...
echo "::notice title=source-recon::recovered $N_JAVA java files from $N_CLASSES classes (vineflower)"
