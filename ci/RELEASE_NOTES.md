# Devouring Storms 10000.0.0-alpha.338 — Build #374 (crash fix + pack summoning + REAL skybox + Story Mode menu + console round 2 + cinematic boot + texture painter + player models + hunter AI)

- **THE RENDER CRASH IS FIXED.** `CancellationException: The call getTextureLocation is not cancellable` — the phase-accurate body texture injection called `setReturnValue` without `cancellable = true`, so every storm body render (including the Picture-in-Picture path) hard-crashed the game. Both that injection and its latent twin in `McsmEntityLightMixin` are now cancellable; a full audit of every cancel/setReturnValue injection in the mod confirmed these were the only two.
- **Built-in pack summoning is RESTORED and now self-proving.** The packs always shipped inside the jar, but the shader-pack toggle defaulted OFF, so every launch the installer ran in uninstall mode (deleted the pack, reset Iris to `(internal)` — your "Loaded Shaderpack: (off) (fallback)"). Now: **Embedded Shader Pack defaults ON** (extracted + auto-selected in Iris's config on first play), the resource packs (Story Look + OG CEM + new Player Models) register as default-enabled built-ins, and if Fabric registration is unavailable the Story Look zip is **summoned** into your resourcepacks folder. Once per world load, chat proves it: `[ds] packs: Story Look: ... | OG CEM: ... | Player Models: ... | shader pack: ...`. Console: new SKY & BUILT-IN PACKS category + a "Re-summon packs + auto-select shader now" action.
- **REAL Story Mode SKYBOX (explicitly NOT a dome).** A textured cube around the camera: six seamless cube faces per phase (verified 12/12 face adjacencies share identical physical edges), converted from equirectangular Story Mode sky panoramas — 1 lavender day / 2 midnight blue / 3 sunset / 4 turquoise / 5 purple / 6 brown-purple witherstorm, the storm eye sitting at the front-centre of the sky. It follows the NEAREST tracked storm's phase (per storm), yaws so the eye faces the storm, pitch-locks the horizon, cross-fades between phase skies and **fades back to the regular vanilla sky** when the storm is gone. Toggle OFF = the vanilla variant. Console: fade seconds + size controls.
- **STORY MODE ACCURATE MAIN MENU (the whole section).** The base's own OG Story Mode panorama + OG sun/moon now renders (the #371 procedural scene is retired as the backdrop), framed Telltale-style: readability vignette, dark translucent episodic panels behind every menu button with gold hover frames + corner ticks, the story-mode banner, the silver border frame, and the "An Episode in Five Acts — The Wither Storm Saga" cinematic bar.
- **Cinematic boot + epic depth animations.** On launch: a 4.2s **side-view cutscene** (the Wither Storm silhouette drifting in and growing, two lightning strikes, the saga title — any key skips), then the **command block grows, glows and BURSTS into the main menu** (expanding ring, 24 radial spokes, fragment spray, flash). On every world load the **world cracks apart** — nine jagged fissures from the centre + two giant **sky shockwave rings** + peak flash. **Every main-menu button breaks apart** on press: an 18-fragment gold shatter spray that flies on over the screen the button opens. Console category VIII CINEMATICS: Cinematic Boot + World Crack Intro toggles.
- **Config console round 2 (more options, persisted).** New chapters: VII SKY & BUILT-IN PACKS and VIII CINEMATICS; STORM PALETTE gains the texture painter entry; GAMEPLAY & SYSTEMS gains Enhanced AI (now persisted — it was saved-but-never-loaded), Giant 3D Preview and the Player Model Versions help. **Shift+A now opens the exact same console as Shift+C.**
- **Giant 3D preview REMOVED from the base config screen by default** (user order) — the new "Giant 3D Preview" toggle brings it back; the rows panel expands into the freed width.
- **IN-GAME STORM TEXTURE PAINTER (make your own textures).** 16×16 classic storm-skin canvas, 20-colour palette, paint/erase by click+drag; Save writes a working resource pack to `resourcepacks/DevouringStormsCustomStorm/` (160×160 body sheet) — enable it in the pack screen and your texture is on the storm.
- **3 PLAYER MODEL VERSIONS** via your installed EMF/CEM (OptiFine-format .jem, the same proven format as the OGS CEM pack): **Telltale Hero** (bigger head, broader body), **Scout** (slim), **Storm Guardian** (brawny) — box-geometry-only with standard 64×64 skin UVs so any skin drapes right. Switch per player with `/scoreboard objectives add mcsmplyr dummy` + `/scoreboard players set @s mcsmplyr 1|2|3|0` (0 = vanilla). Shipped as a default-enabled built-in pack + release zip. (A one-key cycle follows as soon as the 26.2 command API dump returns; true custom NPC entities are a separate modelling effort.)
- **ADVANCED WITHER AI (it hunts you).** With Enhanced AI on, the storm locks its body rotation onto the nearest player every 6 ticks inside a 56-block hunt radius and forces a tentacle slam inside 18 blocks at the console Grab Interval cadence — built only on proven public entity surface, on top of the base's own chase goal.
- Carried from #372: the Story Mode Console UI overhaul, celestial live controls, the self-diagnosing dual-geometry jar audit, gloss kept, OG geometry default, traced-shading default, 133-file jar-overrides map. Technical version locked at `10000.0.0-alpha.338`; display string verbatim `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338`.

# Devouring Storms 10000.0.0-alpha.338 — Build #372 (Story-Mode Console total UI overhaul + self-diagnosing jar audit + celestial controls)

- **The control panel is now the STORY MODE CONSOLE — a total Telltale-style interface overhaul.** The generic blocky vanilla `Button` / `AbstractSliderButton` widgets inside `McsmExtrasScreen` are gone; everything is drawn as clean, minimal, dark translucent episodic panel cards on a deep-charcoal room with a warm amber accent.
  - **Sidebar chapter navigation:** a scrolling vertical navigation rail (CHAPTERS I–VI: Visuals & Sky, Storm Palette, Debris & Physics, Motion, Nightglow & Death, Gameplay & Systems) with active/hover states. It is a data-driven `Category` registry — the structural framework for the upcoming 200+ configuration tabs: add a Category and the sidebar, scrolling, and control rendering grow automatically.
  - **Crisp interactive controls:** boolean rows get a sliding amber pill (instant, live apply); value rows get a **geometric diamond-dot track** (click or drag to scrub in real time) plus a **value chip that opens a text input field** for exact entry (the only vanilla widget instantiated; click-away / Done / close commits, clamped to range).
  - All previous options are present (77 controls), plus the new celestial rows below. Version header locked verbatim: `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338`.
- **Celestial settings are fully unlocked and interactive in real time.** The console's Gameplay & Systems chapter now exposes the sun shadow directly — **Cast A Sun Shadow**, **Shadow Darkness**, **Soft Shadow Edge**, **Shadow Colour Red/Green/Blue** — writing the base `DabyWSClientConfig` fields live (no gate re-run, takes effect immediately, no more stuck pitch-black shadow state). The **MCSM 3D Sun Slab (SAGE MCSM)** toggle + **OG Sun Slab Strength** (0–3) are in Visuals & Sky, also applied live. The base config screen's "The Sun Shadow" rows (resolution, soft edge, cull, heightmap) stay unlocked by the same mechanism (they un-lock whenever `stormShadow` is on).
- **Build #371 failure root-caused and repaired.** The #371 run failed only on the NEW dual-geometry jar audit, which counted `addBox=0 / partDefs=0` with no diagnosable evidence (runner logs are egress-blocked). The audit is now self-diagnosing and grounded: (1) the jar's `WitherStormP4.class` must be byte-identical to the freshly compiled one (`cmp` freshness check — the real stale-class guard), (2) the SOURCE geometry must carry 1700–1800 `addBox` and ≥580 part defs (1259 original + 495 custom = 1754; the 302-name tree twice = 604), (3) the bytecode cross-check only fails when the disassembler actually counts and disagrees; a 0-count with freshness+source passing degrades to a warning, and the full picture (mode, counts, javap head, sample refs) is dumped to `out/P4_AUDIT.txt` and uploaded as an artifact for one-download diagnosis.
- **Gloss patches audited, not purged.** The four `McsmGloss*` mixins were inspected: each targets a different class (`WitherCommandBlock`, `WitherStormDevourer`, `WitherStormP4`, `WitherStormRenderer`), with no overlapping or duplicated injection points and all syntactically valid — they are the approved Build #368 Tattletale gloss feature (user-requested, retained through #369–#371) and are KEPT. The #371 failure was the audit gate, not any gloss code.
- Carried from Build #371: default storm geometry = ORIGINAL base-mod body verbatim (fixes the summon-time vertex-overflow crash), 495-cube Blockbench mesh opt-in and bound strictly to the Netflix/Custom preset path, menu unfreeze (no gate re-arm on panel changes; explicit "Re-apply MCSM Look now"), OG traced-shading body default for phases 4–5.5, OG 3D sun glow restored (no 0.45 ceiling), cinematic story-mode main menu, 133-file jar-overrides asset map, Phase 1–5.5 clean-black / Phase 6+ cosmic texture policy.
- Technical version locked at `10000.0.0-alpha.338`; the console header renders `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338` verbatim.

# Devouring Storms 10000.0.0-alpha.338 — Build #371 (OG geometry default + Netflix preset mesh + menu unfreeze + traced-shading default + OG sun + story-mode main menu)

- **Default storm geometry is the ORIGINAL base-mod body again.** `WitherStormP4` is now DUAL-MODE: `createBodyLayer()` dispatches on a config flag. The DEFAULT (`customMeshModel = false`) is the original base-mod geometry **verbatim** (1,259 cubes / 302 parts) — the shape the base mod shipped with, which fixes the summon-time vertex-overflow crash that the dense rewrite introduced. The clean **495-cube Blockbench/StageB matrix is OPT-IN** and initializes only when the flag is on.
- **The high-detail mesh is bound strictly to the Netflix / Custom preset path.** A new `McsmPresetMeshSyncMixin` hooks `DabyWSClientConfig.applyPreset(...)`, `refreshPreset()` and `load()`: applying the **Netflix (4)** preset turns the custom mesh ON; applying **MCSM OG / Legacy / Cinematic (1–3)** turns it OFF; **Custom (0)** and the panel's "Blockbench Custom Mesh (Preset)" toggle leave it to the user. Everything else (302 part names, constructor, keyframes, tentacle physics, DebrisRing) is preserved in both branches. (The entity model layer is built once per client session on first render, so switching the mesh takes effect from the next world load / relog.)
- **The MCSM Extras menu is UNFROZEN.** Every panel toggle/slider no longer re-arms the force-look gate (the `McsmGate.reset()` calls are removed), so changing a setting applies immediately and **sticks** — it no longer silently wipes base-screen changes or look presets. The gate runs once per session; a new explicit **"Re-apply MCSM Look now"** button (Gameplay tab) is the only way to force the MCSM baseline back on top of your settings (it clears the gate's value memory and re-runs).
- **Sub-phase buttons + 3D preview restored:** the ctor injection that forced `previewShown = false` (which hid the Phase / Sub-phase picker) is removed, so the interactive 3D preview and its Stage sub-phase buttons are shown and toggleable by default. **Sun Shadow** rows (Shadow Map Resolution, Soft Shadow Edge, Cull Back Faces, Heightmap) are live and interactive (the gate keeps `stormShadow` on, which unlocks them), and **Skybox & Atmosphere** controls are fully editable — changes persist instead of reverting.
- **OG traced-shading body is the DEFAULT for phases 4–5.5.** A new 16×16 authentic StageB traced-shading sheet (`wither_storm/wither_storm_traced.png`, extracted from the clean Blockbench source, bilinear-smoothed — no grain) is now the default body texture for phases 4–5.9 via `StormSkins`. The **regular plain deep-black body remains the option** (toggle "OG Traced Shading Body (Ph 4–5.5)" OFF). Phases 1–3.9 keep the clean solid black; Phase 6+ stays HARD-LOCKED to the 4-quadrant cosmic sheet (eyes/teeth emissive isolated).
- **The OG "SAGE MCSM" 3D sun glow is restored and configurable.** The old `sunGlowStrength` 0.45 ceiling (which dimmed the sun) is REMOVED from the gate; the sun now runs at its authentic base default **2.2** (full warm glow) via the internal `storm_sun_glow` post shader. New panel controls: **"OG 3D Sun Glow (SAGE MCSM)"** toggle + **"OG Sun Glow Strength"** slider (0–3), applied live (no gate re-run). The in-game **moon is the vanilla celestial** (the base mod ships no custom moon asset — nothing to update).
- **Cinematic STORY-MODE main menu.** The title backdrop is now a fully procedural cinematic panorama — deep indigo night sky, drifting nebula wisps, twinkling stars, a cool moon, the OG golden 3D sun-slab rising on the horizon with god rays, and three layers of dark mountain silhouettes, framed by the story-mode banner (top) and the "An Episode in Five Acts — The Wither Storm Saga" cinematic bar (bottom). It is built only from `fill`/`fillGradient`/`text` (no texture/asset dependency). Toggle: "Cinematic Story-Mode Menu" (default ON; OFF = the plain gradient).
- **The frozen "Multi-Layer Sky Blending" toggle is now live** — it reads/writes the base `cloudDeckLayer` directly.
- **Hard jar-audit gate updated for dual-mode:** the assembled `WitherStormP4` class must carry BOTH geometry branches — **1,700–1,800 `addBox` invocations** (1,259 original + 495 custom = 1,754) and **≥580 part definitions** (the 302-name tree twice); a stale single-mode or runaway class fails the build.
- Retained: the full 132-file `jar-overrides/` asset map, the Phase 1–5.5 clean-black / Phase 6+ cosmic texture policy, the native gloss sheen, and all gameplay gates. Technical version locked at `10000.0.0-alpha.338`; display string verbatim `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338`.

# Devouring Storms 10000.0.0-alpha.338 — Build #370 (Phase 4/5.5 model rewrite from clean MCSM Blockbench source)

- **The Phase 4–5.5 storm body geometry was rewritten from the clean, un-scrambled MCSM Blockbench source** — `Stage_B/witherstormStageB.bbmodel` (the public asset archive `ALL wither storm assets from MCSM for blockbench you'll ever need V0.9 4`, fetched and parsed in full: 22,211 clean triangles, 40,741 vertices). The old in-game model was a flat, top-truncated hand transcription (650×301×190 slab, ZERO boxes in the top decile of the body — the "holes in Phase 4 / broken top of Phase 5.5" you saw). The new geometry is a deterministic maximal-box reconstruction of the clean trimesh silhouette (649×351×555, full height, full depth): **495 dense boxes, verified 0 silhouette holes and 0 stray overhang** — no phase renders missing chunks or floating blocks.
- **Engine constraint (why boxes):** Minecraft 26.2 entity models are box-only (`ModelPart`/`CubeListBuilder` — verified against the 26.2 client class list; there is no trimesh entity geometry). The clean Blockbench files store triangulated mesh, so the rewrite boxifies the clean silhouette exactly as the base mod's own pipeline expects.
- **Everything behavioral is preserved:** all 302 part names (the keyframe animations bake against them), the constructor, `TentaclePhysics`/`SnatchGrab` (generic tree walk — unaffected), and the DebrisRing subtree are kept verbatim. The old scattered cube lists in the body part tree were emptied; the reconstructed set lives in the `bone` root part at the original pivot (0, -15, 7).
- **Texture binding unchanged (Build #369 policy still in force):** Phases 1–5.5 keep the clean solid deep-black canonical sheet (a uniform texture, so the new geometry renders appearance-complete), Phase 6+ stays hard-locked to the high-res 4-quadrant cosmic sheet, eyes/teeth emissive layers isolated. The Phase 6 devourer geometry (24k-line model with its working cosmic UV layout) is deliberately NOT touched in this build — the rewrite pipeline (`ci/modelgen/boxify.py`) is ready for it once a UV-aware pass is added.
- **Hard audit gate added:** the assembled jar must contain the RECOMPILED `WitherStormP4` class (bytecode-checked: 450–600 `addBox` invocations, ≥290 part definitions, DebrisRing present) — a stale base-jar twin now fails the build instead of silently shipping the old pancake geometry.
- `McsmExtrasScreen.java` untouched (live Sun Shadow slider + Sub-phase buttons exactly as before). Technical version locked at `10000.0.0-alpha.338`; display string verbatim `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338`.

# Devouring Storms 10000.0.0-alpha.338 — Build #369 (Build #364 asset map restored + gloss + strict phase texture policy)

- **Built on the FIRST build's asset map (commit 85ab8d3, Build #364)** — not on the intermediate branch that had purged `jar-overrides/`. The full 132-file asset map is back in the jar: the new "Devouring Storms — The Point of No Return" mod icon, `config/colors.json`, the internal `storm_glow.fsh` / `storm_sun_glow.fsh` shaders, the `mcsm_atmosphere` sky + glare (halo) texture set, the main-menu panorama + title background, the villager recolors, and the canonical `witherstormmod` texture set. The assembly now overlays consolidated src assets first and `jar-overrides/` on top, exactly reproducing the approved Build #364 jar.
- **Menu base from the first build:** `McsmExtrasScreen.java` and `McsmStormBlobMixin.java` restored to the Build #364 versions (full advanced panel with Sun Shadow / Skybox & Atmosphere / Geometry Thinning, header "Open Devouring Storms · 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338", original backdrop pass behavior).
- **Strict opaque texture policy (no holes, no grain):** Phases 1-5 render the clean solid vanilla-black canonical map; Phase 6+ dynamically swaps to the high-resolution 4-quadrant cosmic sheet (`phase_4_assets_p6.png`, devourer pieces to `devourer_assets_p6.png`); eye/teeth emissive maps stay isolated on facial channels.
- **Native Tattletale gloss overlay kept:** the secondary translucent pass (`McsmGlossSheen` + body-model mixins) re-renders the storm body against the scrolling `storm_gloss.png` sheet — shiny out of the box, no external shader pack. Toggle: `native_gloss_sheen` (default ON).
- **Hard jar-audit gates:** every working-tree mod texture must exist in the jar and match the effective source byte-for-byte (jar-overrides wins on overlap), both Phase-6 cosmic sheets must exceed 50 KB, and 12 key restored assets (icon, panorama, title background, colors.json, internal shaders, canonical textures, villager) are individually required — any miss fails the build.
- **Entity geometry / texture binding repair:** the base renderer resolves the body skin through a shared volatile phase hint that off-frame code can pollute (it is stamped with the max phase of every tracked storm, and on the first frame it is 0) — so the Phase 6 cosmic atlas could silently never bind while the body kept the old canonical skin. New `McsmPhaseTextureLockMixin` locks the binding to the entity's actual phase: it stamps this storm's real phase at the start of every render frame, and re-resolves `getTextureLocation` directly from `state.phase` / `state.devourer` (Phase 4-5 → smooth vanilla-black wrap on the actual block shapes, Phase 6+ devourer body → 4-quadrant cosmic sheet, no fallback skins, no stale tiling). `StormSkins` gained a phase-aware `devourer(double)` for that hook. Debris rings are unchanged base-mod code bound to `tractor_beam.png`, which the audit verifies present in the jar.
- Technical version stays `10000.0.0-alpha.338`; the menu option explicitly renders `Open Devouring Storms 10000.0.0-PRE-RELEASE-ALPHA-1-DEVOURING-STORMS-338`.

# Devouring Storms 10000.0.0-alpha.338 — Build #368 (Tattletale gloss + Phase 6 cosmic merge + restored cosmic textures)

- Merged the native Tattletale gloss layer and the Phase 6 cosmic texture rule into the active menu base. `McsmExtrasScreen.java` and the Sun Shadow / Skybox / Geometry Thinning sliders are untouched - the layout is exactly as before, and no code was reverted.
- **Restored the missing internal shader & resource assets from Build #364 (commit 85ab8d3):** all 16 cosmic/phase entity textures under `assets/dabywitherstormmod/textures/entity/` (the `phase_4_assets*` and `devourer_assets*` family, including the 4-quadrant Phase 6 cosmic sheet `phase_4_assets_p6.png`) came back as their full-resolution originals - the jar had been carrying 2-3 KB placeholder canvases for them. The Story Look pack, `shaderpack-v5`, and the Super Duper default shader pack directory maps were verified byte-identical to Build #364 and are embedded as before (built-in `resourcepacks/` + extractable zips + `shaderpacks/devouringstorms.zip`).
- **Asset registration verified by a hard jar-audit gate:** every working-tree mod texture (129 files) must exist in the assembled jar and cmp byte-for-byte against the source, and both Phase-6 cosmic sheets must exceed 50 KB - a placeholder or stale copy now fails the build instead of shipping a flat-colored storm.
...
- **Phase 6 cosmic atlas rule re-confirmed active:** when the storm reaches Phase 6+, `StormSkins` dynamically swaps the body texture to the custom 4-quadrant cosmic sheet (`phase_4_assets_p6.png`); Phases 1-5 keep the smooth solid vanilla-black canonical map. No grainy textures.
