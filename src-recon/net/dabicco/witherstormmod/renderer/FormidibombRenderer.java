...
import net.dabicco.witherstormmod.entity.state.FormidibombRenderState;
...
public class FormidibombRenderer extends EntityRenderer<net.dabicco.witherstormmod.entity.FormidibombEntity, FormidibombRenderState> {
   private static final float BLOCK_SIZE = 4.0F;
   private static final Identifier WHITE = Identifier.fromNamespaceAndPath("dabywitherstormmod", "textures/entity/tractor_beam.png");
   ...
   public void extractRenderState(net.dabicco.witherstormmod.entity.FormidibombEntity entity, FormidibombRenderState state, float partialTick) {
   ...
   public void submit(FormidibombRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
      ...
      if (state.shake > 1.0E-4F) {
         float sx = Mth.sin(state.ticks * 12.9898F + (float)state.seed) * state.shake;
         float sz = Mth.sin(state.ticks * 78.233F + (float)state.seed) * state.shake;
         poseStack.translate(sx, 0.0, sz);
      }
   ...
   private static int lerpLight(int light, float amount) {
      if (amount <= 0.0F) {
         ...
         int b = (int)(block + (240 - block) * amount);
         int s = (int)(sky + (240 - sky) * amount);
         return s << 16 | b;
      }
   }
}
