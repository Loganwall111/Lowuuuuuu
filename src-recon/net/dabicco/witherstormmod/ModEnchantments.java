...
public final class ModEnchantments {
   public static final ResourceKey<Enchantment> GRAVITIC_DRAG = ResourceKey.create(
      Registries.ENCHANTMENT, Identifier.fromNamespaceAndPath("dabywitherstormmod", "gravitic_drag")
   );
   private static final EquipmentSlot[] ARMOUR = new EquipmentSlot[]{EquipmentSlot.HEAD, EquipmentSlot.CHEST, EquipmentSlot.LEGS, EquipmentSlot.FEET};
      ...
      for (EquipmentSlot slot : ARMOUR) {
         ...
         if (!stack.isEmpty()) {
            int level = EnchantmentHelper.getItemEnchantmentLevel(
               entity.level().registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(GRAVITIC_DRAG), stack
            );
            if (level > 0) {
               float scale = Math.min(level, 3) / 3.0F;
               total += 0.125F * scale;
               if (isHeavy(stack)) {
                  total += 0.0375F * scale;
               }
