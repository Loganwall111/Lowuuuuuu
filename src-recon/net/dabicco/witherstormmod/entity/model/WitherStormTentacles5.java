...
public class WitherStormTentacles5 extends EntityModel<WitherStormRenderState> {
   ...
   public void setupAnim(WitherStormRenderState state) {
      ...
      float droop = net.dabicco.witherstormmod.entity.CollapseAnim.droop(state.collapseTicks);
      ...
      if (state.underSiege) {
         ...
         float full = ClientConfigCache.cfg.endermanSiegeTentacleSpeed / 100.0F;
         writhe *= 1.0F + (full - 1.0F) * (0.38F + 0.62F * p);
      }

      float idleTime = state.idleTimeTicks * writhe;
      Integer[] order = new Integer[]{0, 1, 2, 3, 4, 5};
      Random rng = new Random(state.stormId * 2654435761L);
   ...
   public static void bigIdleMath(List<ModelPart> chain, float timeTicks, int idx, float weight) {
      ...
      if (n != 0) {
         ...
         float t = timeTicks * 0.026F * speed + idx * 2.3F;
         float side = 1.0F;
         float heave = (Mth.sin(t * 0.61F + idx * 1.7F) * 0.1F + Mth.sin(t * 0.27F + 2.4F) * 0.05F) * breath;
         float lift = Mth.sin(t * 0.43F + idx * 2.9F) * 0.17F * breath;
         float swing = Mth.sin(t * 0.35F + idx * 1.1F) * 0.2F * breath;
         float prevDrop = 0.0F;
         float prevSide = 0.0F;

         for (int i = 0; i < n; i++) {
            ModelPart bone = chain.get(i);
            float along = n > 1 ? (float)i / (n - 1) : 0.0F;
            float cumDrop = hangDrop(along, heave, lift);
            float cumSide = hangSide(along) * (side + swing);
            float phase = t - i * 0.42F * travel;
            float waveDown = Mth.sin(phase);
            float waveSide = Mth.sin(phase * 0.63F + 1.9F);
   ...
   private static float hangDrop(float along, float heave, float lift) {
      return net.dabicco.witherstormmod.entity.BigTentacleShape.hangDrop(along, heave, lift);
   }
   ...
   private static void sideUncurl(List<ModelPart> chain, float strength) {
      if (!(strength <= 1.0E-4F)) {
         for (int i = 0; i < chain.size(); i++) {
            float along = (float)i / chain.size();
   ...
   public static void smallIdle(List<ModelPart> chain, float timeTicks, int tentacleIdx, float weight) {
      ...
      float t = timeTicks * 0.022F * speed + tentacleIdx * 2.3F;

      for (int i = 0; i < chain.size(); i++) {
         ModelPart bone = chain.get(i);
         float along = (float)i / chain.size();
         float phase = t - i * 0.28F * travel;
         float amp = (0.14F + 0.3F * along) * weight * depth;
         bone.xRot = bone.xRot + Mth.sin(phase) * amp;
         bone.zRot = bone.zRot + Mth.sin(phase * 0.63F + 1.9F) * amp * cross;
      }
   }
   ...
   private boolean ragdoll(List<ModelPart> chain, WitherStormRenderState state, int chainIndex, float droop) {
      if (!(droop <= 0.001F) && !chain.isEmpty()) {
         ...
         if (parent == null) {
            ...
            long key = (long)state.stormId << 8 | chainIndex & 0xFF;
   ...
   public static void limp(List<ModelPart> chain, float amount, float timeTicks, int idx, float groundBias, boolean lengthwiseX) {
      if (!(amount <= 0.001F)) {
         float lean = 0.24F + 0.1F * Mth.sin(idx * 2.399963F);
         float reach = lean * (1.0F + groundBias * 0.55F);
         int n = chain.size();

         for (int i = 0; i < n; i++) {
            ModelPart bone = chain.get(i);
            float along = (float)i / Math.max(1, n - 1);
            float weight = (float)Math.exp(-along * 4.5F);
            float drop = (reach * weight + 0.012F) * amount;
            float settle = 0.055F * (1.0F - weight) * (1.0F + groundBias * 0.8F) * amount;
            drop += settle;
            float wobble = Mth.sin(idx * 1.31F + along * 2.1F) * 0.05F * amount;
            if (lengthwiseX) {
               bone.zRot -= drop;
               bone.yRot += wobble;
            } else {
