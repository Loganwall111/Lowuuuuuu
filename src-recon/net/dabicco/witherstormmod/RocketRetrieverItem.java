...
import net.dabicco.witherstormmod.ModComponents;
import net.dabicco.witherstormmod.entity.GrappledTntEntity;
import net.dabicco.witherstormmod.entity.Yeet;
...
public class RocketRetrieverItem extends Item {
   ...
   public static net.dabicco.witherstormmod.item.RetrieverContents contents(ItemStack stack) {
      return (net.dabicco.witherstormmod.item.RetrieverContents)stack.getOrDefault(
         ModComponents.RETRIEVER_CONTENTS, net.dabicco.witherstormmod.item.RetrieverContents.EMPTY
      );
   }

   private static void setContents(ItemStack stack, net.dabicco.witherstormmod.item.RetrieverContents c) {
      if (c.isEmpty()) {
   ...
   private static int load(ItemStack retriever, ItemStack src) {
      net.dabicco.witherstormmod.item.RetrieverContents c = contents(retriever);
      int take = 0;
      if (src.is(Items.TNT)) {
         ...
         net.dabicco.witherstormmod.item.RetrieverContents next = c;
   ...
   public boolean releaseUsing(ItemStack stack, Level level, LivingEntity entity, int timeLeft) {
      if (entity instanceof Player player) {
         if (player.getMainHandItem() != stack && player.getOffhandItem() != stack) {
            ...
            net.dabicco.witherstormmod.item.RetrieverContents c = contents(stack);
            if (!c.canShoot()) {
               return false;
   ...
   public Optional<TooltipComponent> getTooltipImage(ItemStack stack) {
      net.dabicco.witherstormmod.item.RetrieverContents c = contents(stack);
      return c.isEmpty() ? Optional.empty() : Optional.of(new net.dabicco.witherstormmod.item.RetrieverTooltip(c.tnt(), c.rocketCount()));
   }
}
