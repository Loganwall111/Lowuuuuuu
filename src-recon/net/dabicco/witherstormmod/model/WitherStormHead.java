...
public class WitherStormHead extends EntityModel<WitherStormHeadRenderState> {
   ...
   public void setupAnim(WitherStormHeadRenderState state) {
      ...
      this.lowerJaw.xRot = this.lowerJaw.xRot + (float)Math.toRadians(state.jawAngle);
      this.lowerJaw.yRot = this.lowerJaw.yRot + (float)Math.toRadians(state.jawLagYaw);
      this.lowerJaw.xRot = this.lowerJaw.xRot + (float)Math.toRadians(state.jawLagPitch);
      this.lowerJaw.zRot = this.lowerJaw.zRot + (float)Math.toRadians(state.jawLagRoll);
      if (state.upsideDown != 0.0F) {
         this.root.zRot = this.root.zRot + (float)Math.toRadians(state.upsideDown);
         this.root.y += 39.8F;
      }
   }
