...
public class BowelsTentacleRenderer extends EntityRenderer<net.dabicco.witherstormmod.bowels.BowelsTentacleEntity, BowelsTentacleRenderer.State> {
   ...
   public BowelsTentacleRenderer(Context context) {
      super(context);
      this.modelSet = context.getModelSet();
   }

   public BowelsTentacleRenderer.State createRenderState() {
      return new BowelsTentacleRenderer.State();
   }

   public void extractRenderState(net.dabicco.witherstormmod.bowels.BowelsTentacleEntity entity, BowelsTentacleRenderer.State state, float partialTick) {
      ...
      if (mc.player != null && entity.getGrabPrompt() > 0 && entity.getGrabFor() == mc.player.getId()) {
         state.prompt = 'Q';
         state.promptLeft = entity.getGrabPrompt() / 40.0F;
   ...
   public void submit(BowelsTentacleRenderer.State state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
      ...
      poseStack.pushPose();
      poseStack.translate(state.mount.x, 0.0, state.mount.z);
