...
public final class ActionButtonRender {
   ...
   public static void submit(PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera, char letter, float left, float side) {
      submit(poseStack, collector, camera, letter, left, side, 0.0);
   }
   ...
   public static void submit(PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera, char letter, float left, float side, double atHeight) {
      ...
      Font font = mc.font;
      poseStack.pushPose();
      poseStack.translate(2.4F * side, 1.6F + atHeight, 0.0);
      ...
      if (letter != 0) {
         ...
         collector.submitText(
            poseStack, -font.width(text) / 2.0F, -4.0F, FormattedCharSequence.forward(text, Style.EMPTY), false, DisplayMode.SEE_THROUGH, 15728880, -1, 0, 0
         );
         poseStack.popPose();
      }
   ...
   private static void quad(Pose pose, VertexConsumer buffer, float half, int argb) {
