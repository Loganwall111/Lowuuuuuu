...
public final class BowelsDebug {
   ...
   public static void tick(Minecraft client) {
      if (ENABLED) {
         ...
         if (player != null && client.level != null && BowelsGravity.isBowels(client.level) && client.level.getGameTime() % 20L == 0L) {
            ...
            System.out
               .printf(
                  "[bowels] of=%s box=%s step=%s noGrav=%s ground=%s%n         pos=(%.2f %.2f %.2f)%n         aabb=%s%n         eye=(%.2f %.2f %.2f) cam=(%.2f %.2f %.2f) apart=%.2f%n         vel=(%.4f %.4f %.4f)%n",
                  of,
                  box,
                  step,
                  player.isNoGravity(),
