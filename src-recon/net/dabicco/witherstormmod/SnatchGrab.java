...
public final class SnatchGrab {
   ...
   private static final Map<Integer, net.dabicco.witherstormmod.client.SnatchGrab.Grab> GRABS = new HashMap<>();

   private SnatchGrab() {
   }

   public static void apply(ModelPart root, WitherStormRenderState state) {
      net.dabicco.witherstormmod.client.SnatchGrab.Grab grab = GRABS.get(state.stormId);
      if (state.snatchActive || grab != null && !(grab.blend <= 0.0F)) {
         List<net.dabicco.witherstormmod.client.TentaclePhysics.Chain> chains = net.dabicco.witherstormmod.client.TentaclePhysics.chainsFor(root);
         if (!chains.isEmpty()) {
            if (grab == null) {
               grab = new net.dabicco.witherstormmod.client.SnatchGrab.Grab();
            ...
            if (grab.chainIdx >= 0 && grab.chainIdx < chains.size() && !(grab.blend <= 0.001F)) {
               reachChain(chains.get(grab.chainIdx), grab.aim, grab.blend, state.idleTimeTicks);
               if (GRABS.size() > 32) {
                  GRABS.clear();
               }
            }
   ...
   public static void reachUnderStorm(ModelPart tentaclesRoot, WitherStormRenderState state) {
      if (state.snatchActive) {
         List<net.dabicco.witherstormmod.client.TentaclePhysics.Chain> chains = net.dabicco.witherstormmod.client.TentaclePhysics.chainsFor(tentaclesRoot);
         if (!chains.isEmpty()) {
            Matrix3f model = new Matrix3f()
               .rotateY((float)Math.toRadians(180.0F - state.bodyRot))
               .rotateZ((float)Math.toRadians(-state.bodyRoll))
            ...
            if (!(aim.lengthSquared() < 1.0E-4F)) {
               ...
               if (idx >= 0) {
                  reachChain(chains.get(idx), aim, 1.0F, state.idleTimeTicks);
               }
            }
         }
      }
   ...
   private static int pickChain(List<net.dabicco.witherstormmod.client.TentaclePhysics.Chain> chains, Vector3f targetDir) {
      float maxLen = 0.0F;

      for (net.dabicco.witherstormmod.client.TentaclePhysics.Chain c : chains) {
      ...
      for (int i = 0; i < chains.size(); i++) {
         net.dabicco.witherstormmod.client.TentaclePhysics.Chain c = chains.get(i);
   ...
   private static void reachChain(net.dabicco.witherstormmod.client.TentaclePhysics.Chain chain, Vector3f aimDir, float blend, float timeTicks) {
      ...
      for (int i = 0; i < n; i++) {
         ...
         restSeg.normalize();
         float along = n > 1 ? (float)i / (n - 1) : 1.0F;
         float curl = blend * (0.15F + 0.85F * along);
         float w = 0.06F * Mth.sin(timeTicks * 0.2F + i * 0.5F) * blend;
