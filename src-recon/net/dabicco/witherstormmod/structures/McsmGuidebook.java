...
public final class McsmGuidebook {
   ...
   private static boolean hasBook(ServerPlayer player) {
      for (ItemStack s : player.getInventory()) {
         if (s.is(Items.WRITTEN_BOOK)) {
            WrittenBookContent c = (WrittenBookContent)s.get(DataComponents.WRITTEN_BOOK_CONTENT);
            if (c != null && "The Order of the Stone".equals(c.title().raw())) {
               return true;
            }
         }
      }
   ...
   public static ItemStack create() {
      ...
      WrittenBookContent content = new WrittenBookContent(Filterable.passThrough("The Order of the Stone"), "Dabby's Wither Storm", 0, pages, true);
   ...
   private static Component cmd(String command, String label) {
      return Component.literal(command)
         .withStyle(Style.EMPTY.withColor(ChatFormatting.DARK_AQUA).withUnderlined(true).withClickEvent(new SuggestCommand(command)))
   ...
   private static Component tp(String coords) {
      ...
      return Component.literal("[ Teleport there ]")
         .withStyle(Style.EMPTY.withColor(ChatFormatting.DARK_AQUA).withBold(true).withClickEvent(new SuggestCommand(command)));
   }
}
