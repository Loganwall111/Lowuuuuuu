...
public final class StormGlowRenderer {
   ...
   public static void submitLight(
      PoseStack poseStack, SubmitNodeCollector collector, Vec3 centre, Vec3 view, double radius, float[] sizes, float[] alphas, int[][] colours, float amount
   ) {
      if (!(amount <= 0.004F) && !(radius <= 0.001)) {
         ...
         for (int i = sizes.length - 1; i >= 0; i--) {
            ...
            if (alpha > 2) {
               double r = radius * sizes[i];
               Vec3 rx = right.scale(r);
               Vec3 uy = up.scale(r);
               int[] c = colours[i];
               collector.submitCustomGeometry(poseStack, net.dabicco.witherstormmod.client.GlowRenderTypes.glow(GLOW_SPRITE), (pose, consumer) -> {
   ...
   private static void vertex(Pose pose, VertexConsumer consumer, Vec3 at, float u, float v, int[] rgb, int alpha) {
