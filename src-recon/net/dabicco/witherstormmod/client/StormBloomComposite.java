...
public final class StormBloomComposite {
   ...
   private static RenderPipeline pipeline() {
      if (pipeline == null) {
         pipeline = RenderPipeline.builder(new Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()})
            .withLocation(Identifier.fromNamespaceAndPath("dabywitherstormmod", "pipeline/storm_bloom_add"))
            .withVertexShader(Identifier.withDefaultNamespace("core/screenquad"))
            .withFragmentShader(Identifier.fromNamespaceAndPath("dabywitherstormmod", "post/storm_bloom_add"))
            .withBindGroupLayout(BindGroupLayout.builder().withSampler("InSampler").withSampler("DepthSampler").build())
            .withBindGroupLayout(BindGroupLayout.builder().withUniform("BloomConfig", UniformType.UNIFORM_BUFFER).build())
   ...
   private static void draw(RenderTarget scene, RenderTarget glow, GpuBuffer ubo) {
      RenderPass pass = RenderSystem.getDevice()
         ...
         .createRenderPass(
            () -> "dabyws storm bloom composite", scene.getColorTextureView(), Optional.empty(), scene.getDepthTextureView(), OptionalDouble.empty()
         );
