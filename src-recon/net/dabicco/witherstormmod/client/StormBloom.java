...
public final class StormBloom {
   private static final Identifier[][] ENTITY_CHAINS = new Identifier[][]{
      {
            id("storm_bloom_entity_subtle_t0"),
            id("storm_bloom_entity_subtle_t1"),
            id("storm_bloom_entity_subtle_t2"),
            id("storm_bloom_entity_subtle_t3"),
            id("storm_bloom_entity_subtle_t4")
      },
      {id("storm_bloom_entity_t0"), id("storm_bloom_entity_t1"), id("storm_bloom_entity_t2"), id("storm_bloom_entity_t3"), id("storm_bloom_entity_t4")},
      {
            id("storm_bloom_entity_strong_t0"),
            id("storm_bloom_entity_strong_t1"),
            id("storm_bloom_entity_strong_t2"),
            id("storm_bloom_entity_strong_t3"),
   ...
   private static final double REFERENCE_DISTANCE = 40.0;
   ...
   private static final double[] TIER_DISTANCES = new double[]{30.0, 60.0, 110.0, 200.0};
   ...
   private static float apparentScale(Minecraft mc) {
      ...
      if (best == Double.MAX_VALUE) {
         ...
         double dist = Math.max(Math.sqrt(best), 1.0);
         return (float)Mth.clamp(40.0 / dist, 0.12, 1.6);
      }
   }
   ...
   public static boolean wantsEntityTarget() {
      return !failed
         ...
         && DabyWSClientConfig.bloomMaskToStorm
         && Math.round(DabyWSClientConfig.bloomStrength) > 0L;
   }
   ...
   public static void beginFrame() {
      net.dabicco.witherstormmod.client.StormSceneDepth.beginFrame();
      net.dabicco.witherstormmod.client.StormRenderStats.report();
      net.dabicco.witherstormmod.client.StormShadowMap.beginFrame();
      if (wantsEntityTarget()) {
         Minecraft mc = Minecraft.getInstance();
         if (mc.level != null) {
            net.dabicco.witherstormmod.client.StormBloomTarget.beginFrame(mc);
         }
      }
   }
   ...
   public static void process() {
      if (!failed) {
         if (net.dabicco.witherstormmod.client.ShaderPackCompat.active()) {
            ...
            if (level <= 0) {
               ...
               if (mc.level != null && mc.player != null) {
                  try {
                     if (DabyWSClientConfig.bloomMaskToStorm) {
                     ...
                     failed = true;
                     drivingOutlineTarget = false;
                     System.out.println("[dabywitherstormmod] storm bloom DISABLED after an error: " + var3);
   ...
   private static RenderTarget sceneDepthSource(Minecraft mc) {
      ...
      return net.dabicco.witherstormmod.client.StormBloomTarget.capturedScene();
   }

   public static void noteDepthCaptureFailure(Throwable t) {
      if (!depthCaptureFailure) {
         depthCaptureFailure = true;
         System.out.println("[dabywitherstormmod] scene depth capture FAILED, bloom occlusion is off (glow will show through geometry): " + t);
      }
   }

   private static boolean processHeadsOnly(Minecraft mc, int level) {
      RenderTarget heads = net.dabicco.witherstormmod.client.StormBloomTarget.target();
      if (heads == null) {
         ...
         if (mainTarget == null || heads.width == mainTarget.width && heads.height == mainTarget.height) {
            int debug = (int)Math.round(DabyWSClientConfig.bloomDebug);
            net.dabicco.witherstormmod.client.StormBloomDiag.tick();
            net.dabicco.witherstormmod.client.StormBloomDiag.setEnabled(debug != 0);
            ...
            if (debug != 0 && !sizeLine.equals(lastSizeLine)) {
               ...
               System.out.println("[dabywitherstormmod][diag] targets: bloom " + sizeLine);
            }

            net.dabicco.witherstormmod.client.StormBloomProbe.probe(heads);
            int lvl = Math.min(level - 1, 2);
            RenderTarget main = mc.gameRenderer.mainRenderTarget();
            float scale = apparentScale(mc) * (main.height / 1080.0F);
            RenderTarget sceneDepth = sceneDepthSource(mc);
            if (sceneDepth == null && !warnedNoDepth) {
               warnedNoDepth = true;
               System.out.println("[dabywitherstormmod] no scene depth available -- bloom occlusion is OFF, so the glow will show through geometry");
            }

            net.dabicco.witherstormmod.client.StormBloomHdr.run(
               heads,
               sceneDepth != null ? sceneDepth : main,
               sceneDepth,
               Math.max(1.0F, 7.0F * scale),
               Math.max(2.0F, 22.0F * scale),
               EXPOSURE[lvl],
               TIGHT_WEIGHT[lvl],
               WIDE_WEIGHT[lvl],
               debug
            ...
            status(
               "skipping one frame: bloom buffer is " + heads.width + "x" + heads.height + " but the screen is " + mainTarget.width + "x" + mainTarget.height
            );
            return false;
         }
      }
   ...
   private static void processWholeScreen(Minecraft mc, int level) {
      ...
      if (chain == null) {
         status("chain " + chainId + " failed to load (see the errors above this line)");
      } else {
         status("running " + chainId + " (whole screen)");
   ...
   private static void status(String s) {
      if (!s.equals(lastStatus)) {
         lastStatus = s;
         System.out.println("[dabywitherstormmod] storm bloom: " + s);
      }
   }
