...
public final class StormShadowMap {
   ...
   private static final double LIGHT_DISTANCE = 260.0;
   ...
   private static float[] verts = new float[49152];
   ...
   private static final double TERRAIN_MOVE = 6.0;
   ...
   private static final Map<ModelPart, net.dabicco.witherstormmod.client.StormShadowMap.PartGeom> GEOM = new IdentityHashMap<>();
   ...
   public static void beginFrame() {
      faceCulling(net.dabicco.witherstormmod.client.StormShadow.sunDirectionF());
      vertCount = 0;
      haveGeometry = false;
   }
   ...
   public static void captureTerrain(ClientLevel level, Vec3 centre, Vec3 eye, float extent) {
      ...
      boolean moved = Double.isNaN(terrainAtX) || Math.abs(centre.x - terrainAtX) > 6.0 || Math.abs(centre.z - terrainAtZ) > 6.0;
      if (!moved && now - terrainBuiltAt < 500L && terrainCache != null) {
         ...
         int step = TERRAIN_STEP;

         while (reach * 2 / step + 1 > 192) {
            step *= 2;
         }
   ...
   private static void emitTerrainCache(Vec3 eye) {
      if (terrainCache != null) {
         ...
         terrainVertCount = 0;
         terrainMinX = (float)(terrainOriginX - eye.x);
         terrainMinZ = (float)(terrainOriginZ - eye.z);
         terrainMaxX = terrainMinX + (cells - 1) * TERRAIN_STEP;
         terrainMaxZ = terrainMinZ + (cells - 1) * TERRAIN_STEP;

         for (int ix = 0; ix < cells - 1; ix++) {
            for (int iz = 0; iz < cells - 1; iz++) {
               float x0 = (float)(terrainOriginX + ix * TERRAIN_STEP - eye.x);
               float z0 = (float)(terrainOriginZ + iz * TERRAIN_STEP - eye.z);
               float x1 = x0 + TERRAIN_STEP;
               float z1 = z0 + TERRAIN_STEP;
               float h = Math.min(
                  Math.min(terrainCache[ix * cells + iz], terrainCache[(ix + 1) * cells + iz]),
                  Math.min(terrainCache[ix * cells + iz + 1], terrainCache[(ix + 1) * cells + iz + 1])
               );
               float y = (float)(h - eye.y);
               pushTerrain(x0, y, z0);
               pushTerrain(x1, y, z0);
               pushTerrain(x1, y, z1);
               pushTerrain(x0, y, z1);
   ...
   public static boolean wanted() {
      return !failed
         && (DabyWSClientConfig.stormShadow || DabyWSClientConfig.stormSelfShadow)
         && DabyWSClientConfig.stormShadowStrength > 0.0
         && !net.dabicco.witherstormmod.client.ShaderPackCompat.active();
   }
   ...
   public static void capture(PoseStack pose, ModelPart part) {
      if (wanted()) {
         try {
            ...
            failed = true;
            System.out.println("[dabywitherstormmod] storm shadow capture FAILED, shadows off: " + var3);
         }
      }
   }

   private static net.dabicco.witherstormmod.client.StormShadowMap.PartGeom geomFor(ModelPart part) {
      net.dabicco.witherstormmod.client.StormShadowMap.PartGeom hit = GEOM.get(part);
      if (hit != null) {
         return hit;
      } else {
         if (GEOM.size() > 2048) {
         ...
         net.dabicco.witherstormmod.client.StormShadowMap.PartGeom g = new net.dabicco.witherstormmod.client.StormShadowMap.PartGeom();
         g.quads = quads;
         ...
         for (Cube cube : cubes) {
            for (Polygon poly : ((CubePolygonsAccessor)cube).dabyws$getPolygons()) {
   ...
   private static void walk(ModelPart part, PoseStack pose) {
      if (part.visible) {
         ...
         net.dabicco.witherstormmod.client.StormShadowMap.PartGeom g = geomFor(part);
         if (g.quads > 0) {
            float a00 = m.m00();
            float a01 = m.m01();
            float a02 = m.m02();
      ...
      if (failed) {
         ...
         try {
            ...
            if (terrainVertCount > 0) {
               boolean groundStale = groundDrawnAt != terrainBuiltAt
                  || groundDrawnEye == null
                  || groundDrawnEye.distanceToSqr(centre.x, centre.y, centre.z) > 1.0;
               ensureGroundTarget();
               ...
               groundViewProj.setOrtho(-groundHalf, groundHalf, -groundHalf, groundHalf, 0.0F, 1024.0F, true)
                  .lookAt(above, groundAt, new Vector3f(0.0F, 0.0F, -1.0F));
               if (groundStale) {
                  drawGround();
                  groundDrawnAt = terrainBuiltAt;
                  groundDrawnEye = new Vec3(centre.x, centre.y, centre.z);
               }
            ...
            failed = true;
            System.out.println("[dabywitherstormmod] storm shadow map FAILED, shadows off: " + var19);
   ...
   private static void uploadAndDraw() {
      AutoStorageIndexBuffer indexer = RenderSystem.getSequentialBuffer(PrimitiveTopology.QUADS);
      ...
      if (stormVbo != null) {
         ...
         try {
            ...
            pass.setVertexBuffer(0, stormVbo.slice(0L, vertCount * 3 * 4));
   ...
   private static void drawGround() {
      ...
      if (groundVbo != null) {
         ...
         RenderPass pass = RenderSystem.getDevice()
            .createCommandEncoder()
            .createRenderPass(
               () -> "dabyws ground height map",
               groundTarget.getColorTextureView(),
               Optional.of(new Vector4f(0.0F, 0.0F, 0.0F, 1.0F)),
               groundTarget.getDepthTextureView(),
               OptionalDouble.of(1.0)
            );
         ...
         try {
            ...
            pass.setVertexBuffer(0, groundVbo.slice(0L, terrainVertCount * 3 * 4));
            pass.setIndexBuffer(indices, indexer.type());
   ...
   private static GpuBuffer writeVerts(GpuBuffer existing, String name, float[] src, int count) {
      ...
      if (bytes == 0) {
         ...
         if (existing == null || existing.size() < bytes) {
            if (existing != null) {
               existing.close();
            }

            buffer = RenderSystem.getDevice().createBuffer(() -> name, 40, bytes * 2L);
         }

         RenderSystem.getDevice().createCommandEncoder().writeToBuffer(buffer.slice(0L, bytes), vertexStaging);
         return buffer;
      }
   }
   ...
   private static GpuBuffer uploadMatrix(String name, Matrix4f matrix, float kind) {
      ...
      data.rewind();
      return net.dabicco.witherstormmod.client.GpuBufferPool.write(name, 128, data);
   }

   private static RenderPipeline pipeline() {
      if (pipeline == null) {
         pipeline = RenderPipeline.builder(new Snippet[]{RenderPipelinesAccessor.dabyws$globalsSnippet()})
            .withLocation(id("pipeline/storm_shadow_depth"))
            .withVertexShader(id("core/storm_shadow_depth"))
            .withFragmentShader(id("core/storm_shadow_depth"))
            .withVertexBinding(0, DefaultVertexFormat.POSITION)
            .withPrimitiveTopology(PrimitiveTopology.QUADS)
            .withBindGroupLayout(BindGroupLayout.builder().withUniform("ShadowCasterConfig", UniformType.UNIFORM_BUFFER).build())
            .withCull(false)
   ...
   public static float resolution() {
      return builtSize > 0 ? builtSize : size();
   }
   ...
   public static void close() {
      ...
      net.dabicco.witherstormmod.client.GpuBufferPool.close();
   }
