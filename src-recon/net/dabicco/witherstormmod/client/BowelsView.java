...
public final class BowelsView {
   ...
   private static final double TURN_SECONDS = 1.5;
   private static final double ROLL_SHAKE_SECONDS = 0.9;
   private static final double ROLL_TURN_SECONDS = 4.0;
   ...
   private static double shakeSeconds() {
      return isRoll() ? 0.9 : 0.5;
   }

   private static double turnSeconds() {
      return isRoll() ? 4.0 : 1.5;
   }
      ...
      if (level() != null && gravity != null) {
         ...
         if (arriving == Direction.DOWN && leaving == Direction.DOWN) {
            ...
            if (startedAt == 0L) {
               ...
               double since = (now - startedAt) / 1.0E9;
               if (since >= shakeSeconds() + turnSeconds()) {
                  ...
                  float gust = 0.55F + 0.45F * Mth.sin((float)(since * 3.7));
                  float a = strength * gust;
                  float w1 = (float)(since * 9.1);
                  float w2 = (float)(since * 14.3);
                  float w3 = (float)(since * 5.3);
                  SHAKE.rotationX(Mth.sin(w1) * a + Mth.sin(w2 * 0.61F) * a * 0.45F)
                     .rotateZ(Mth.cos(w1 * 0.83F) * a + Mth.sin(w3) * a * 0.55F)
                     .rotateY(Mth.sin(w3 * 0.47F) * a * 0.7F);
                  if (since < shakeSeconds()) {
   ...
   public static boolean holding() {
      return startedAt != 0L && arriving != null && leaving != arriving ? (System.nanoTime() - startedAt) / 1.0E9 < shakeSeconds() : false;
   }
