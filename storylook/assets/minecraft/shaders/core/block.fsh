...
void main() {
    ...
    // Exact fog: blend the game's fog toward the sampled horizon haze.
    vec3 F = FogColor.rgb;
    float fl = dot(F, vec3(0.2126, 0.7152, 0.0722));
    float fnight = 1.0 - smoothstep(0.05, 0.22, fl);
    float fdawn = clamp((F.r - F.b) * 2.2, 0.0, 1.0) * (1.0 - fnight);
    float fday = (1.0 - fnight) * (1.0 - fdawn);
    vec3 phor = fday * vec3(0.420, 0.790, 0.940)
              + fdawn * vec3(0.890, 0.680, 0.730)
              + fnight * vec3(0.019, 0.031, 0.130);
    vec4 fogCol = vec4(mix(F, phor, 0.55), FogColor.a);
