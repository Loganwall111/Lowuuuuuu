...
void main() {
    ...
    float ringWave = sin(angle * 9.0 + time * 1.25 + sin(angle * 3.0) * 2.2) * 0.055;
    float ringRadius = 0.735 + ringWave + sin(time * 0.7 + Seed) * 0.012;
    ...
    rim *= ring * (0.60 + broken * 0.75) * (0.75 + Pulse * 0.45);

    float halo = 1.0 - smoothstep(0.66, 1.04, radius);
    vec3 color = cosmic * interior + rim + energyPalette(fract(angle * 0.09 + time * 0.02)) * halo * 0.045;
    float alpha = clamp(max(interior * 0.92, ring * (0.72 + broken * 0.28)) * vertexColor.a, 0.0, 0.97);
    if (alpha < 0.012) {
        discard;
    }
