...
void main() {
    ...
    vec3 color = mix(FOG_DARK, FOG_SIFT, deep * (0.66 + movingFog * 0.30));
    color += vec3(0.012, 0.018, 0.010) * movingFog;
    ...
    for (int index = 0; index < 9; index++) {
        ...
        float shaft = smoothstep(width, 0.0, abs(p.x - center));
    ...
    float horizon = exp(-abs(p.y + 0.22) * 4.2) * (0.02 + deep * 0.07);
    color += vec3(0.12, 0.20, 0.13) * horizon;
