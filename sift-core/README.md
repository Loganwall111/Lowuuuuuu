...
- The server synchronously warms a 3x3 destination chunk window before sending the respawn packet; The Sift uses a sparse flat generator, so this is a small deterministic preparation rather than a terrain-generation wait.
...
- `SiftTerrainSpawner` adds deterministic floating shelves made from Siftstone, Sift Moss, and Rift Crystal as the player falls. Irregular outlines, underside teeth, and occasional neighbouring-cell shelves avoid a repeated disk silhouette while the open routes preserve the original free-fall character.
