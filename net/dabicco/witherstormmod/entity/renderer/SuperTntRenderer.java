...
import net.dabicco.witherstormmod.entity.state.SuperTntRenderState;
...
public class SuperTntRenderer extends EntityRenderer<net.dabicco.witherstormmod.entity.SuperTntEntity, SuperTntRenderState> {
   ...
   public void extractRenderState(net.dabicco.witherstormmod.entity.SuperTntEntity entity, SuperTntRenderState state, float partialTick) {
   ...
   public void submit(SuperTntRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
      ...
      float scale = 4.0F;
      if (flash > 0.0F) {
         float wobble = 1.0F + Mth.sin(state.fuse * 1.5F) * 0.1F * flash;
         scale = 4.0F * wobble;
      }
