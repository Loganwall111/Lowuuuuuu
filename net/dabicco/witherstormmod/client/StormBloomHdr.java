...
public final class StormBloomHdr {
   ...
   private static RenderPipeline maskPipeline() {
      if (maskPipeline == null) {
         maskPipeline = RenderPipeline.builder(new RenderPipeline.Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()}).withLocation(id("pipeline/storm_hdr_mask")).withVertexShader(Identifier.withDefaultNamespace("core/screenquad")).withFragmentShader(id("post/storm_hdr_mask")).withBindGroupLayout(BindGroupLayout.builder().withSampler("InSampler").withSampler("BloomDepthSampler").withSampler("SceneDepthSampler").withUniform("MaskConfig", UniformType.UNIFORM_BUFFER).build()).withColorTargetState(new ColorTargetState(Optional.empty(), GpuFormat.RGBA16_FLOAT, 15)).build();
      }
   ...
   private static void mask(RenderTarget source, RenderTarget scene, RenderTarget dst, float mode) {
      ...
      try {
         RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "dabyws bloom mask", dst.getColorTextureView(), Optional.empty());
   ...
   private static RenderPipeline debugPipeline() {
      if (debugPipeline == null) {
         debugPipeline = RenderPipeline.builder(new RenderPipeline.Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()}).withLocation(id("pipeline/storm_hdr_debug")).withVertexShader(Identifier.withDefaultNamespace("core/screenquad")).withFragmentShader(id("post/storm_hdr_debug")).withBindGroupLayout(BindGroupLayout.builder().withSampler("InSampler").withUniform("DebugConfig", UniformType.UNIFORM_BUFFER).build()).build();
      }
   ...
   private static void show(GpuTextureView view, RenderTarget scene, float gain, boolean markHdr, boolean depthView) {
      ...
      try {
         RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "dabyws bloom debug", scene.getColorTextureView(), Optional.empty());
   ...
   private static RenderPipeline blurPipeline() {
      if (blurPipeline == null) {
         blurPipeline = RenderPipeline.builder(new RenderPipeline.Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()}).withLocation(id("pipeline/storm_hdr_blur")).withVertexShader(Identifier.withDefaultNamespace("core/screenquad")).withFragmentShader(id("post/storm_hdr_blur")).withBindGroupLayout(BindGroupLayout.builder().withSampler("InSampler").withUniform("BlurConfig", UniformType.UNIFORM_BUFFER).build()).withColorTargetState(new ColorTargetState(Optional.empty(), GpuFormat.RGBA16_FLOAT, 15)).build();
      }
   ...
   private static RenderPipeline compositePipeline() {
      if (compositePipeline == null) {
         compositePipeline = RenderPipeline.builder(new RenderPipeline.Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()}).withLocation(id("pipeline/storm_hdr_composite")).withVertexShader(Identifier.withDefaultNamespace("core/screenquad")).withFragmentShader(id("post/storm_hdr_composite")).withBindGroupLayout(BindGroupLayout.builder().withSampler("TightSampler").withSampler("WideSampler").withUniform("BloomConfig", UniformType.UNIFORM_BUFFER).build()).withColorTargetState(new ColorTargetState(BlendFunction.ADDITIVE)).build();
      }
   ...
   private static void blur(RenderTarget src, RenderTarget dst, float dirX, float dirY, float radius) {
      ByteBuffer data = staging((new Std140SizeCalculator()).putVec4().get());
      Std140Builder.intoBuffer(data).putVec4(dirX / (float)dst.width, dirY / (float)dst.height, radius, 0.0F);
      data.rewind();
      GpuBuffer ubo = RenderSystem.getDevice().createBuffer(() -> "dabyws blur config", 128, data);

      try {
         RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "dabyws bloom blur", dst.getColorTextureView(), Optional.empty());
   ...
   public static void run(RenderTarget source, RenderTarget scene, RenderTarget sceneDepth, float tightRadius, float wideRadius, float exposure, float tightWeight, float wideWeight, int debugStage) {
      ...
      if (debugStage == 1) {
         ...
         if (StormBloomDiag.wanted()) {
            mask(source, sceneDepthOrSelf, tightA, 2.0F);
            StormBloomDiag.report(tightA, source, sceneDepth);
         }
               ...
               if (debugStage == 6) {
                  ...
                  if (debugStage == 7) {
                     ...
                     if (debugStage == 8) {
                        ...
                        try {
                           RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "dabyws bloom composite", scene.getColorTextureView(), Optional.empty());
