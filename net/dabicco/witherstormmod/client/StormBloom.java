...
public final class StormBloom {
   ...
   public static boolean wantsEntityTarget() {
      return !failed && !ShaderPackCompat.active() && DabyWSClientConfig.bloomMaskToStorm && Math.round(DabyWSClientConfig.bloomStrength) > 0L;
   }
            ...
            if (level <= 0) {
               ...
               if (mc.level != null && mc.player != null) {
                  try {
                     ...
                     failed = true;
                     drivingOutlineTarget = false;
                     System.out.println("[dabywitherstormmod] storm bloom DISABLED after an error: " + String.valueOf(e));
                     e.printStackTrace();
                  }
   ...
   public static void noteDepthCaptureFailure(Throwable t) {
      if (!depthCaptureFailure) {
         ...
         System.out.println("[dabywitherstormmod] scene depth capture FAILED, bloom occlusion is off (glow will show through geometry): " + String.valueOf(t));
      }
   }
