...
public final class CaveRumbleClient {
   ...
   private static float envelope(float elapsed) {
      if (startTick != Long.MIN_VALUE && !(elapsed < 0.0F) && !(elapsed >= (float)duration)) {
         float in = Mth.clamp(elapsed / 10.0F, 0.0F, 1.0F);
         float out = Mth.clamp(((float)duration - elapsed) / 40.0F, 0.0F, 1.0F);
   ...
   public static float[] offset(float partialTick) {
      ...
      if (mc.level != null && startTick != Long.MIN_VALUE) {
         ...
         if (env <= 0.001F) {
            ...
            float a = 1.35F * env;
            return new float[]{a * (Mth.sin((double)(elapsed * 1.31F)) * 0.6F + Mth.sin((double)(elapsed * 2.77F)) * 0.4F), a * (Mth.sin((double)(elapsed * 1.09F)) * 0.6F + Mth.sin((double)(elapsed * 3.13F)) * 0.4F), a * 0.7F * (Mth.sin((double)(elapsed * 0.83F)) * 0.6F + Mth.sin((double)(elapsed * 2.29F)) * 0.4F)};
         }
      } else {
         return null;
      }
