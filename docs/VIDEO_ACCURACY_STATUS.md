...
## Batch 15 — MCSM Atmosphere (new)

Fully wired this batch (client FX + server world key + config-menu sections):

- **Twinkling starfield skybox** — `client/StormStarfield.java`: a seeded dome of individually twinkling stars in the storm-blacked sky ("Storm Nights") or every night ("Every Night"); violet stars mixed in, teal wash with the phase-5 palette.
- **MCSM cloud deck** — `client/StormCloudDeck.java`: soft slabs slowly orbiting each storm (stateless per-storm hash placement, no popping), phase-palette tinted, breathing with the pulse. New `textures/misc/mcsm_cloud.png`.
- **Atmospheric pulse** — `client/StormPresenceFX.java`: the phase-5 purple-blue PULSE lives in the *atmosphere*, bigger than the body, wobbling slowly, on its own clock; optional heartbeat thump at each peak; deepens purple at 5.8+.
- **Cataclysm halo pair (phase 5.8+)** — blue-purple halo ring around the area + original white halo underneath the body (`textures/misc/halo_ring.png`).
- **Black glare ring + cluster ejecta** — black-purple glare hugging the silhouette via the new fogless alpha-blend pipeline `GlowRenderTypes.translucent`, with turquoise/green cluster sparks ejecting from the rim.
- **Phase fog palettes** — `client/StormPalettes.java` drives `StormSkyDarken` fog: dark purple pre-5, turquoise at phase 5, purple-black from phase 5.8.
- **OG skins** — `client/StormSkins.java` + `stormSkin` client cycle swaps every storm renderer (body/head/severed/cross-dimensional/nether-scale/distant) between Classic and Obsidian Gloss: glossy near-black with purple sheen, command block belly redrawn as obsidian-purple tiles (`textures/entity/*_og.png`; classic files untouched).
- **Structure tear clusters** — world key `structureTearClusters`: structure raids rip chunks out of buildings as flying clusters, so builds visibly break open and pieces fly up to the storm (`WitherStormEntity.raidStructureTick`).
- **Config menu** — new "Skybox & Atmosphere" master (Stars, Cloud Deck, The Pulse, Cataclysm Halos, Black Glare & Ejecta, Phase Palettes, ~30 new keys); live "Find settings" search filtering all tabs; new "Cinematic" preset (stars every night, dense deck, halos + glare + heartbeat); MCSM preset now includes OG skin + the atmosphere FX; debris size range raised to 3x.

All textures are generated by `tools/gen_storm_textures.py` (pure stdlib).
