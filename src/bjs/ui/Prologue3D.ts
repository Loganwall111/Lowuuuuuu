...
export class Prologue3D {
 ...
 private render=()=>{if(!this.scene||!this.camera)return;const t=(performance.now()-this.startAt)/1000;
  if(t<10)this.phaseEarth(t);else if(t<18)this.phaseMatrix(t-10);else if(t<27)this.phaseWormhole(t-18);else if(t<35)this.phaseHands(t-27);else{this.scene.clearColor=new Color4(1,1,1,1);this.setGroup(this.hands,false);}
  this.scene.render();};
 private phaseEarth(t:number):void{if(this.pilot){this.pilot.position.x=-12+Math.min(1,t/5.5)*19;this.pilot.rotation.y=Math.sin(t*7)*.08;}if(this.rocket&&t>5.2)this.rocket.position.y=2+Math.pow((t-5.2)/4.8,2)*130;if(this.camera)this.camera.position.x=Math.sin(t*.22)*1.2;}
