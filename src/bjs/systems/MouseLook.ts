 ...
 * shader tracking arrays — a locked mouse can only ever pivot the view,
...
export class MouseLook {
   ...
   * one raw hardware delta onto one absolute camera-rotation angle. This is
   * the only mouse-to-look path that touches the camera: movementX/movementY
   * are copied in verbatim (clamped only against synthetic/garbage events)
   * and never reinterpreted as velocity, never matched to screen pixels, and
   * never fed to any shader or background tracking array.
   */
  private rawDx = 0;
  private rawDy = 0;
  ...
  attach(el: HTMLElement): void {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      // ONLY hardware deltas are read here. Absolute cursor coordinates are
      // deliberately never touched, so no input can translate the starfield
      // or a planet: the mouse can only ever pivot the look matrices.
      const CAP = 400;
      ...
      // The raw-delta lane for pointer lock, consumed by consumeSteer().
    ...
    const onLockError = () => {
      ...
      // permission denied, another element holds the lock). Drag-look stays
      // available; the next click inside a fresh user gesture retries.
      this.locked = false;
    };
