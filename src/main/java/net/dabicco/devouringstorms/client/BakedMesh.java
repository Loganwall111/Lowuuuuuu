...
public final class BakedMesh {
 ...
 private static Mesh load(String n) { try { Optional<Resource> r=Minecraft.getInstance().getResourceManager().getResource(Identifier.fromNamespaceAndPath("devouringstorms","meshes/"+n+".json")); if(r.isEmpty()) return new Mesh(new float[0],new float[0],new float[0]); JsonObject o=JsonParser.parseReader(new InputStreamReader(r.get().open())).getAsJsonObject(); return new Mesh(values(o.getAsJsonArray("tris")),values(o.getAsJsonArray("uvs")),values(o.getAsJsonArray("normals"))); } catch(Exception e) { return new Mesh(new float[0],new float[0],new float[0]); } }
