...
public final class TentacleRagdoll {
   ...
   private static final Map<Long, Rope> ROPES = new HashMap();
   ...
   private static void step(Rope rope, Vector3f[] rest, Level level, float dt) {
      int n = rope.x.length;
      if (!(dt <= 0.0F)) {
         float keep = (float)Math.pow((double)0.12F, (double)dt);
         float g = 22.0F * dt * dt;
         ...
         for(int pass = 0; pass < 6; ++pass) {
            for(int i = 0; i < n - 1; ++i) {
               ...
               var24[i] += dz * corr * wA;
               var24 = rope.x;
               var24[i + 1] -= dx * corr * wB;
               var24 = rope.y;
               var24[i + 1] -= dy * corr * wB;
               var24 = rope.z;
               var24[i + 1] -= dz * corr * wB;
            }
   ...
   private static void collide(Rope rope, Level level) {
   ...
   private static float groundAt(Level level, float x, float z) {
      BlockPos pos = level.getHeightmapPos(Types.MOTION_BLOCKING_NO_LEAVES, BlockPos.containing((double)x, (double)0.0F, (double)z));
   ...
   private static Vector3f[] restLayout(List<ModelPart> chain, Matrix4f toWorld, Quaternionf ancestorQ) {
      ...
      for(int i = 0; i < chain.size(); ++i) {
         ModelPart bone = (ModelPart)chain.get(i);
      ...
      if (chain.size() >= 2) {
         ModelPart last = (ModelPart)chain.get(chain.size() - 1);
   ...
   private static void apply(List<ModelPart> chain, Rope rope, Matrix4f toWorld, Quaternionf ancestorQ, float amount) {
      ...
      for(int i = 0; i < chain.size(); ++i) {
         ...
         if (!(desired.lengthSquared() < 1.0E-8F)) {
            ...
            if (i + 1 < chain.size()) {
               ModelPart child = (ModelPart)chain.get(i + 1);
   ...
   public static Matrix4f parentTransform(ModelPart modelRoot, ModelPart target, Quaternionf outQ) {
      ...
      if (path.isEmpty()) {
         ...
         for(int i = 0; i < path.size() - 1; ++i) {
            ModelPart p = (ModelPart)path.get(i);
