...
public class ClientConfigCommandHandler {
   public static void handle(Minecraft mc, ClientConfigCommandPayload payload) {
      if (mc.player != null) {
         switch (payload.mode()) {
            case 0:
               DevouringStormsClientConfig.Key key = (DevouringStormsClientConfig.Key)DevouringStormsClientConfig.KEYS.get(payload.key());
               if (key == null) {
                  return;
               }

               MutableComponent line = Component.literal(key.name() + " = ").withStyle(ChatFormatting.GRAY).append(Component.literal(format(key, key.get().getAsDouble())).withStyle(ChatFormatting.WHITE));
               ...
               break;
            case 1:
               DevouringStormsClientConfig.Key key1 = (DevouringStormsClientConfig.Key)DevouringStormsClientConfig.KEYS.get(payload.key());
               if (key1 == null) {
                  return;
               }
   ...
   private static String format(DevouringStormsClientConfig.Key key, double value) {
      if (key.toggle()) {
         return value >= (double)0.5F ? "on" : "off";
      } else {
