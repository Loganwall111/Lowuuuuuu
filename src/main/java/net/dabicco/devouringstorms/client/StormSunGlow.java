...
public final class StormSunGlow {
   ...
   private static RenderPipeline pipeline() {
      if (pipeline == null) {
         pipeline = RenderPipeline.builder(new RenderPipeline.Snippet[]{RenderPipelinesAccessor.dabyws$postProcessingSnippet()}).withLocation(id("pipeline/storm_sun_glow")).withVertexShader(Identifier.withDefaultNamespace("core/screenquad")).withFragmentShader(id("post/storm_sun_glow")).withBindGroupLayout(BindGroupLayout.builder().withSampler("DepthSampler").withUniform("SunGlowConfig", UniformType.UNIFORM_BUFFER).build()).withColorTargetState(new ColorTargetState(BlendFunction.ADDITIVE)).build();
      }
   ...
   public static void render(CameraRenderState camera) {
      if (!failed && camera != null) {
         if (DevouringStormsClientConfig.sunGlow) {
            if (!ShaderPackCompat.active()) {
               float gloom = StormSkyDarken.factor();
               float strength = gloom * (float)DevouringStormsClientConfig.sunGlowStrength;
               if (!(strength <= 0.002F)) {
                        ...
                        if (!(strength <= 0.002F)) {
                           ...
                           if (scene != null && scene.useDepth && scene.getDepthTextureView() != null) {
                              ...
                              try {
                                 ...
                                 Std140Builder.intoBuffer(data).putMat4f(invViewProj).putVec4((float)sun.x, (float)sun.y, (float)sun.z, strength).putVec4((float)DevouringStormsClientConfig.sunGlowR, (float)DevouringStormsClientConfig.sunGlowG, (float)DevouringStormsClientConfig.sunGlowB, 120.0F);
                                 data.rewind();
                                 GpuBuffer ubo = GpuBufferPool.write("dabyws sun glow cfg", 128, data);
                                 RenderPass pass = RenderSystem.getDevice().createCommandEncoder().createRenderPass(() -> "dabyws sun glow", scene.getColorTextureView(), Optional.empty());
                                 ...
                                 failed = true;
                                 System.out.println("[devouringstorms] sun glow DISABLED after an error: " + String.valueOf(e));
                                 e.printStackTrace();
                              }
