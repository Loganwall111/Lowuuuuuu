...
import net.dabicco.devouringstorms.mixin.CubePolygonsAccessor;
import net.dabicco.devouringstorms.mixin.ModelPartAccessor;
import net.dabicco.devouringstorms.mixin.ModelPartCubesAccessor;
...
public final class StormShadowMap {
   ...
   private static int size() {
      return (int)DevouringStormsClientConfig.shadowMapResolution;
   }

   private static int terrainStep() {
      return DevouringStormsClientConfig.stormShadowTerrain ? 2 : 8;
   }

   public static void faceCulling(Vector3fc sun) {
      cullAway = sun != null && DevouringStormsClientConfig.shadowCullBackFaces;
      if (cullAway) {
         cullSunX = sun.x();
         cullSunY = sun.y();
         cullSunZ = sun.z();
   ...
   public static void status(String reason) {
      if (!reason.equals(lastStatus)) {
         lastStatus = reason;
         System.out.println("[devouringstorms][shadow] " + reason);
      }
   }
   ...
   public static void capture(PoseStack pose, ModelPart part) {
      if (wanted()) {
         try {
            ...
            failed = true;
            System.out.println("[devouringstorms] storm shadow capture FAILED, shadows off: " + String.valueOf(e));
         }
   ...
   public static boolean build(Vector3fc sun, Vector3f centre, float radius) {
      if (failed) {
         ...
         try {
            ...
            failed = true;
            System.out.println("[devouringstorms] storm shadow map FAILED, shadows off: " + String.valueOf(e));
            e.printStackTrace();
            return false;
         }
      }
