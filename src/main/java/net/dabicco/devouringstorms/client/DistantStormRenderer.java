...
public class DistantStormRenderer {
   ...
   private static double squashDistance(Minecraft mc) {
      ...
      double factor = DevouringStormsClientConfig.distantFog ? 0.995 : 0.55;
   ...
   public static void render(LevelRenderContext ctx) {
      ...
      if (mc.level != null) {
         boolean renderDistantModels = DevouringStormsClientConfig.distantStorms;
         ensureInit(mc);
         Vec3 cam = ctx.levelState().cameraRenderState.pos;
         PoseStack pose = ctx.poseStack();
         SubmitNodeCollector consumers = ctx.submitNodeCollector();
         boolean optimize = DevouringStormsClientConfig.optimizeDistantAnimations;
         ...
         for(ClientDistantStormManager.StormData d : ClientDistantStormManager.all()) {
            ...
            if (!(dist < (double)1.0F)) {
               boolean legacy = DevouringStormsClientConfig.legacyDistantRenderer;
