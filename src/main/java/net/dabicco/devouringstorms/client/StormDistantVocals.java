...
public final class StormDistantVocals {
   ...
   public static void tick(Minecraft mc) {
      if (mc.player != null && mc.level != null && DevouringStormsClientConfig.stormAmbience) {
         if (!mc.isPaused()) {
               ...
               if (nextIn > 0) {
                  ...
                  if (!found) {
                     ...
                     if (!(dist < (double)60.0F) && !(dist > (double)500.0F)) {
                        ...
                        float userVol = (float)Math.max((double)0.0F, DevouringStormsClientConfig.headSoundsVolume);
