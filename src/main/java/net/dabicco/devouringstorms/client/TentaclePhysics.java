...
import net.dabicco.devouringstorms.mixin.ModelPartAccessor;
...
public final class TentaclePhysics {
   ...
   private static final Map<ModelPart, List<Chain>> CHAINS = new HashMap();
   private static final Map<Long, Sim> SIMS = new HashMap();

   private static float gravity() {
      return (float)DevouringStormsClientConfig.verletGravity;
   }

   private static float sway() {
      return (float)DevouringStormsClientConfig.verletSway;
   }

   private static float damping() {
      return (float)DevouringStormsClientConfig.verletDamping;
   }

   private static float writhe() {
      return (float)DevouringStormsClientConfig.verletWrithe;
   }

   private static float writheSpeed() {
      return (float)DevouringStormsClientConfig.verletWritheSpeed;
   }

   static List<Chain> chainsFor(ModelPart root) {
      return (List)CHAINS.computeIfAbsent(root, TentaclePhysics::discoverChains);
   }
   ...
   public static void apply(ModelPart root, WitherStormRenderState state, int namespace, float maxDeviation) {
      List<Chain> chains = chainsFor(root);
      if (!chains.isEmpty()) {
         ...
         for(int c = 0; c < chains.size(); ++c) {
            ...
            Sim sim = (Sim)SIMS.computeIfAbsent((long)state.stormId << 12 | (long)namespace << 8 | (long)c, (k) -> newSim(chain));
   ...
   public static void curlAndVanish(ModelPart root, float progress) {
      if (!(progress <= 0.0F)) {
         for(Chain chain : chainsFor(root)) {
   ...
   private static Sim newSim(Chain chain) {
      Sim sim = new Sim();
   ...
   private static void step(Chain chain, Sim sim, Vector3f gravity, Vector3f sway, float now, int chainIdx) {
      ...
      if (!(dt <= 0.0F)) {
         ...
         for(int i = 1; i < n; ++i) {
            ...
            float along = (float)i / (float)n;
            float amp = writhe() * (0.35F + 0.65F * along);
            float p1 = now * writheSpeed() - (float)i * 0.16F + (float)chainIdx * 2.1F;
            float p2 = now * writheSpeed() * 0.55F - (float)i * 0.11F + (float)chainIdx * 4.7F;
            float wx = (Mth.sin((double)p1) + 0.6F * Mth.sin((double)(p2 + 0.9F))) * amp;
            float wz = (Mth.cos((double)(p1 * 0.9F + 1.3F)) + 0.6F * Mth.cos((double)p2)) * amp * 0.85F;
            float wy = Mth.sin((double)(p2 + (float)chainIdx)) * amp * 0.6F;
   ...
   private static void limitReach(Chain chain, Sim sim, float maxDeviation) {
      ...
      for(int i = 1; i < sim.pos.length; ++i) {
         ...
         if (!(d2 <= maxSq) && !(d2 < 1.0E-8F)) {
            float scale = maxDeviation / (float)Math.sqrt((double)d2);
   ...
   private static void pose(Chain chain, Sim sim) {
   ...
   private static List<Chain> discoverChains(ModelPart root) {
   ...
   private static void collect(ModelPart part, List<Chain> out) {
      for(ModelPart child : children(part).values()) {
         ...
         ModelPart cur;
         for(cur = child; children(cur).size() == 1; cur = (ModelPart)children(cur).values().iterator().next()) {
            run.add(cur);
         }
   ...
   private static Chain buildChain(List<ModelPart> run) {
      Chain chain = new Chain();
      chain.bones = (ModelPart[])run.toArray(new ModelPart[0]);
