...
public final class StormMusic {
   ...
   public static void tick(Minecraft mc) {
      if (mc.level != null && mc.player != null) {
         if (!DevouringStormsClientConfig.stormMusic) {
            stopAll(mc);
   ...
   private static boolean audible(Minecraft mc, ClientDistantStormManager.StormData storm) {
      double range = DevouringStormsClientConfig.stormMusicRange;
      if (mc.player.position().distanceTo(new Vec3(storm.x, storm.y, storm.z)) > range) {
         return false;
      } else {
         int cutoff = (int)DevouringStormsClientConfig.stormMusicCaveCutoff;
         if (cutoff <= 0) {
            return true;
         } else {
   ...
   static float gain() {
      return (float)Mth.clamp(DevouringStormsClientConfig.stormMusicVolume, (double)0.0F, (double)2.0F);
   }
