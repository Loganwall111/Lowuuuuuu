...
import net.dabicco.devouringstorms.mixin.RenderTypeInvoker;
...
public final class GlowRenderTypes {
   ...
   public static RenderType bloomEraseOccluded(Identifier texture) {
      return (RenderType)ERASE_TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_bloom_erase:" + String.valueOf(tex), RenderSetup.builder(erasePipeline()).withTexture("Sampler0", tex).useLightmap().setOutputTarget(StormBloomTarget.eraseOutputTarget()).createRenderSetup()));
   }
   ...
   public static RenderType bloomOccluder(Identifier texture) {
      return (RenderType)OCCLUDER_TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_bloom_occluder:" + String.valueOf(tex), RenderSetup.builder(occluderPipeline()).withTexture("Sampler0", tex).setOutputTarget(StormBloomTarget.outputTarget()).useLightmap().createRenderSetup()));
   }

   public static RenderType bloomSource(Identifier texture) {
      return (RenderType)BLOOM_TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_bloom_src:" + String.valueOf(tex), RenderSetup.builder(bloomSourcePipeline()).withTexture("Sampler0", tex).setOutputTarget(StormBloomTarget.outputTarget()).createRenderSetup()));
   }

   public static RenderType emitterMark(Identifier texture) {
      return ShaderPackCompat.active() ? RenderTypes.eyes(texture) : (RenderType)MARK_TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_emitter_mark:" + String.valueOf(tex), RenderSetup.builder(markPipeline()).withTexture("Sampler0", tex).createRenderSetup()));
   }

   public static RenderType glow(Identifier texture) {
      return (RenderType)TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_glow:" + String.valueOf(tex), RenderSetup.builder(pipeline()).withTexture("Sampler0", tex).createRenderSetup()));
   }
   ...
   public static RenderType translucent(Identifier texture) {
      return (RenderType)TRANSLUCENT_TYPES.computeIfAbsent(texture, (tex) -> RenderTypeInvoker.dabyws$create("devouringstorms:storm_translucent:" + String.valueOf(tex), RenderSetup.builder(translucentPipeline()).withTexture("Sampler0", tex).createRenderSetup()));
   }
