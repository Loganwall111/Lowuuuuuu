...
public final class StormPulseFX {
   ...
   public static void trigger(int entityId, double x, double y, double z, float phase, int kind) {
      ...
      if (mc.player != null && mc.level != null) {
         double range = kind == KIND_SUMMON ? Math.max(24.0, DevouringStormsClientConfig.pulseHeartbeatRange * 1.2) : Math.max(16.0, DevouringStormsClientConfig.pulseHeartbeatRange);
         double dist = mc.player.position().distanceTo(new Vec3(x, y, z));
         float vol = (float)(1.0 - 0.82 * Mth.clamp(dist / range, 0.0, 1.0));
         if (vol > 0.03F) {
            mc.getSoundManager().play(new StormPulseSound(x, y, z, kind == KIND_SUMMON ? vol * 1.15F : vol));
            if (kind == KIND_SUMMON) {
   ...
   public static void render(LevelRenderContext ctx) {
      ...
      while (it.hasNext()) {
         ...
         if ((!summon && !DevouringStormsClientConfig.atmospherePulse) || (summon && !DevouringStormsClientConfig.summonShockwave)) {
            continue;
         }

         float lifeTicks = BASE_LIFE_TICKS * (summon ? 1.18F : Mth.clamp((float)DevouringStormsClientConfig.pulsePeriod / 4.0F, 0.55F, 2.0F));
         ...
         float growIn = Math.max(summon ? 10.0F : 14.0F, lifeTicks * (summon ? 0.18F : 0.24F));
         float fadeStart = Math.max(growIn + 2.0F, lifeTicks * (summon ? 0.30F : 0.22F));
         ...
         fade *= fade;
         float amount = (summon ? (float)DevouringStormsClientConfig.summonShockwaveStrength : (float)DevouringStormsClientConfig.pulseStrength) * fade;
         if (amount <= 0.004F) {
            continue;
         }
         ...
         double bodyR = (summon ? 32.0 : 24.0) + Math.max(0.0F, pulse.phase() - 5.0F) * (summon ? 12.0 : 10.0);
         double size = summon ? DevouringStormsClientConfig.summonShockwaveSize : DevouringStormsClientConfig.pulseSize;
         double radius = bodyR * (summon ? 1.85 + 2.35 * grow : 1.4 + 1.7 * grow) * size;
         Vec3 centre = rel.add(0.0, bodyR * (summon ? 0.12 : 0.15), 0.0);
         StormGlowRenderer.submitLight(pose, collector, centre, view, radius, LAYER_SIZES, LAYER_ALPHAS, layerCols, summon ? amount * 1.08F : amount);

         int alpha = (int)((summon ? 210.0F : 180.0F) * fade);
         ring(pose, collector, centre, view, radius * 0.96, radius * (summon ? 0.22 : 0.18), layerCols[0][0], layerCols[0][1], layerCols[0][2], alpha);
         ring(pose, collector, centre, view, radius * 1.18, radius * (summon ? 0.12 : 0.10), 255, 255, 255, (int)((summon ? 145.0F : 110.0F) * fade));
      }
   }
