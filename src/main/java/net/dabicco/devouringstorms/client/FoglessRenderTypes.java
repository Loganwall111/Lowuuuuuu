...
public final class FoglessRenderTypes {
   ...
   public static boolean fogless() {
      return active && !DevouringStormsClientConfig.legacyDistantRenderer && !ShaderPackCompat.active();
   }

   private static float fogMix() {
      return DevouringStormsClientConfig.distantFog ? 0.5F : 0.0F;
   }

   public static boolean modelShading() {
      return DevouringStormsClientConfig.stormModelShading && !ShaderPackCompat.active();
   }
   ...
   public static boolean reverseShading() {
      return DevouringStormsClientConfig.reverseShading && !ShaderPackCompat.active();
   }
   ...
   private static RenderType cached(String kind, Identifier tex, float fog, RenderPipeline pipeline, boolean lightmap) {
      return (RenderType)TYPES.computeIfAbsent(kind + "|" + String.valueOf(tex) + "|" + fog + "|" + String.valueOf(pipeline.getLocation()), (k) -> {
         ...
         return RenderTypeInvoker.dabyws$create("devouringstorms:" + kind + ":" + String.valueOf(tex) + ":" + tag(fog), b.createRenderSetup());
      });
   }
