...
import net.dabicco.devouringstorms.client.TractorBeamRenderer;
...
public final class PreviewHeads {
   ...
   public static float lookYaw(float timeTicks, int head) {
      float t = timeTicks * 0.034F + (float)head * 2.7F;
      return 30.0F * (0.62F * Mth.sin((double)(t * 0.53F)) + 0.38F * Mth.sin((double)(t * 1.17F + 1.9F)));
   }

   public static float lookPitch(float timeTicks, int head) {
      float t = timeTicks * 0.034F + (float)head * 4.1F;
      return 17.0F + 16.0F * (0.65F * Mth.sin((double)(t * 0.41F + 2.3F)) + 0.35F * Mth.sin((double)(t * 0.87F)));
   }

   private static float jaw(float timeTicks, int head) {
      float t = timeTicks * 0.034F + (float)head * 1.6F;
      return 2.4F + 2.4F * Mth.sin((double)(t * 0.33F));
   }
   ...
   public void submit(
      ...
      float emit = (float)Mth.clamp(DevouringStormsClientConfig.glowStrength, 0.0, 1.0);
      ...
      for (int i = 0; i < count; i++) {
         ...
         float roll = restRoll[i] * (0.65F + 0.35F * Mth.sin((double)(idleTimeTicks * 0.017F + (float)i * 2.4F)));
         poseStack.pushPose();
         ...
         if (firing) {
            ...
            double dirX = (double)(-Mth.cos((double)py) * Mth.sin((double)yy));
            double dirY = (double)(-Mth.sin((double)py));
            double dirZ = (double)(Mth.cos((double)py) * Mth.cos((double)yy));
            double reach = ((double)scene.groundY - off.y) / dirY;
            ...
            float radius = (float)ClientConfigCache.cfg.beamGroundRadius;
         ...
         if (!shadowPass && emit > 0.02F) {
            ...
            int gr = (int)((float)(tint >> 16 & 0xFF) * emit);
            int gg = (int)((float)(tint >> 8 & 0xFF) * emit);
            int gb = (int)((float)(tint & 0xFF) * emit);
            collector.submitModel(
               this.glow[i],
               head,
               poseStack,
