...
import net.dabicco.devouringstorms.entity.model.SeveredWitherStorm;
...
public class SeveredWitherStormRenderer extends MobRenderer<SeveredWitherStormEntity, SeveredWitherStormRenderState, SeveredWitherStorm> {
   ...
   protected AABB getBoundingBoxForCulling(SeveredWitherStormEntity entity) {
   ...
   protected int getModelTint(SeveredWitherStormRenderState state) {
      if (this.previewShadowPass) {
         return 940578856;
      }
      int argb = super.getModelTint(state);
      float white = Mth.clamp(state.collapseWhiteout, 0.0F, 1.0F);
   ...
   public void extractRenderState(SeveredWitherStormEntity entity, SeveredWitherStormRenderState state, float partialTick) {
      ...
      state.droop = CollapseAnim.droop(state.collapseTicks);
   ...
   private static float settleSlope(int entityId, boolean pitch, float target) {
      ...
      if (slot == null) {
         ...
         slot[1] = (float)now;
         float ease = 1.0F - (float)Math.exp((double)(-dt * 2.5F));
      ...
      if (state.preview != null) {
         this.previewHeads
            .submit(
               ...
               SeveredWitherStormEntity.previewHeadOffsets(state.mirrored),
               SeveredWitherStormEntity.previewHeadScales(),
               new float[3],
               new float[3],
               state.bodyRot,
               state.idleTimeTicks,
   ...
   protected void scale(SeveredWitherStormRenderState state, PoseStack poseStack) {
      ...
      float topple = CollapseAnim.severedPitch(state.collapseTicks);
      float spin = CollapseAnim.severedSpin(state.collapseTicks, state.side);
      float sideRoll = CollapseAnim.severedRoll(state.collapseTicks, state.side);
      ...
      if (state.droop > 0.0F) {
         poseStack.translate(0.0, 2.0 * (double)state.droop, 0.0);
      }
