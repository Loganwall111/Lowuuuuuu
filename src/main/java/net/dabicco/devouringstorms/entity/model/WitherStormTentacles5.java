...
import net.dabicco.devouringstorms.client.SnatchGrab;
import net.dabicco.devouringstorms.client.TentacleRagdoll;
import net.dabicco.devouringstorms.config.DevouringStormsClientConfig;
import net.dabicco.devouringstorms.entity.animation.PhaseStormAnim;
import net.dabicco.devouringstorms.entity.BigTentacleShape;
import net.dabicco.devouringstorms.entity.CollapseAnim;
import net.dabicco.devouringstorms.entity.animation.WitherStormTentacles5Animation;
...
public class WitherStormTentacles5 extends EntityModel<WitherStormRenderState> {
   ...
   public void setupAnim(WitherStormRenderState state) {
      ...
      float writhe = state.phase >= 5.8 ? (float)DevouringStormsClientConfig.lateGrowthWrithe : 1.0F;
      if (state.underSiege) {
   ...
   public static void bigIdleMath(List<ModelPart> chain, float timeTicks, int idx, float weight) {
      int n = chain.size();
      if (n != 0) {
         float speed = (float)DevouringStormsClientConfig.tentacleIdleSpeed * PhaseStormAnim.speed();
         float travel = (float)DevouringStormsClientConfig.tentacleWaveTravel * PhaseStormAnim.speed();
         float depth = (float)DevouringStormsClientConfig.bigTentacleCurlDepth * PhaseStormAnim.depth();
         float breath = (float)DevouringStormsClientConfig.bigTentacleHangBreath * PhaseStormAnim.breath();
   ...
   private static float hangSide(float along) {
      ...
      return 0.85F * smoothstep(u) * (float)DevouringStormsClientConfig.bigTentacleSideSweep;
   }
   ...
   public static void smallIdle(List<ModelPart> chain, float timeTicks, int tentacleIdx, float weight) {
      float speed = (float)DevouringStormsClientConfig.tentacleIdleSpeed * PhaseStormAnim.speed();
      float travel = (float)DevouringStormsClientConfig.tentacleWaveTravel * PhaseStormAnim.speed();
      float depth = (float)DevouringStormsClientConfig.tentacleCurlDepth * PhaseStormAnim.depth();
      float cross = (float)DevouringStormsClientConfig.tentacleCrossAxis;
