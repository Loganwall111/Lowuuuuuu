...
public final class SeveredRope {
   ...
   public void tick(Level level, double weight, Vec3 mouth, Vec3 anchor) {
      ...
      double gravity = 0.055 * (hauled ? 0.18 : (double)1.0F);
      ...
      for(int i = 0; i < this.now.length; ++i) {
         ...
         if (!hauled && this.onGround(level, this.now[i], weight)) {
            velocity = velocity.multiply(0.55, (double)1.0F, 0.55);
         }

         Vec3 next = this.now[i].add(velocity).add((double)0.0F, weight * gravity, (double)0.0F);
         if (hauled) {
            Vec3 toward = mouth.subtract(this.now[i]);
            double d = toward.length();
            if (d > 1.0E-4) {
               next = next.add(toward.scale(0.06 * ((double)1.0F - 0.8 * (double)i / (double)last) / d));
            }
         }
      ...
      for(int pass = 0; pass < 6; ++pass) {
         for(int i = 0; i < this.length.length; ++i) {
            ...
            if (!(d < 1.0E-6)) {
               double push = (d - this.length[i]) * (double)0.5F;
      ...
      for(int i = 0; i < this.now.length; ++i) {
         ...
         if (!level.getBlockState(at).isAir() && level.getBlockState(at).isSolidRender()) {
            if (weight < (double)0.0F) {
               double top = (double)at.getY() + 1.02;
               if (p.y < top) {
                  this.now[i] = new Vec3(p.x, top, p.z);
               }
            } else {
               double under = (double)at.getY() - 0.02;
               if (p.y > under) {
   ...
   private boolean onGround(Level level, Vec3 p, double weight) {
      BlockPos below = BlockPos.containing(p.x, p.y + (weight < (double)0.0F ? -0.08 : 0.08), p.z);
      return level.getBlockState(below).isSolidRender();
   }

   public float[][] joints(int from) {
      float[][] out = new float[BowelsTentacleShape.BONES][2];
      ...
      for(int i = 0; i + 1 < this.now.length; ++i) {
         ...
         double len = d.length();
         if (!(len < 1.0E-6)) {
            float pitch = (float)(-Math.asin(Mth.clamp(d.y / len, (double)-1.0F, (double)1.0F)));
            float yaw = (float)Math.atan2(d.x, d.z);
            out[bone][0] = pitch - lastPitch;
            out[bone][1] = Mth.degreesDifference((float)Math.toDegrees((double)lastYaw), (float)Math.toDegrees((double)yaw)) * ((float)Math.PI / 180F);
            lastPitch = pitch;
            lastYaw = yaw;
         }
      }
