...
public final class BowelsTentacleShape {
   ...
   public static double reachTo(int bone) {
      double at = (double)0.0F;

      for(int i = 0; i < Math.min(bone, BONE_LEN.length); ++i) {
         at += (double)BONE_LEN[i] / (double)16.0F;
      }
   ...
   public static float[][] joints(int bones, float curl, float time, float phase, float aimPitch, float aimYaw, float aim, float open, float sway, float rigid, float coil) {
      return joints(bones, curl, time, phase, aimPitch, aimYaw, aim, open, sway, rigid, coil, 0.0F);
   }

   public static float[][] joints(int bones, float curl, float time, float phase, float aimPitch, float aimYaw, float aim, float open, float sway, float rigid, float coil, float wrap) {
      bones = Mth.clamp(bones, 1, BONES);
      float[][] out = new float[BONES][2];

      for(int i = 0; i < bones; ++i) {
         float along = (float)i / (float)(BONES - 1);
         ...
         float huntX = Mth.lerp(along, -0.155F, 0.055F) + Mth.sin((double)(wave - (float)i * 0.55F)) * 0.085F;
         float huntY = Mth.cos((double)(wave * 0.8F - (float)i * 0.4F + 0.5F)) * 0.11F;
         ...
         if (aim > 0.0F && i > 0) {
            float perJoint = (aimPitch - out[0][0]) / (float)(bones - 1);
            x = Mth.lerp(aim, x, perJoint);
            y = Mth.lerp(aim, y, aimYaw / (float)(bones - 1));
         }

         if (sway > 0.0F) {
            float rate = (0.082F + Math.abs(phase) % 1.0F * 0.03F) * (1.0F + 0.55F * Math.max(0.0F, sway - 1.0F));
            float slow = Mth.sin((double)(time * rate + phase + (float)i * 0.3F));
            float fast = Mth.sin((double)(time * rate * 2.37F + phase * 2.1F + (float)i * 0.52F));
            ...
            x -= (rear + snap(Mth.sin((double)(time * rate * 1.63F + phase * 1.3F + (float)i * 0.22F))) * 0.2F) * sway * reach;
         }
   ...
   public static Vec3[] path(int bones, float scale, float[][] joints) {
      ...
      double[] m = new double[]{(double)1.0F, (double)0.0F, (double)0.0F, (double)0.0F, (double)1.0F, (double)0.0F, (double)0.0F, (double)0.0F, (double)1.0F};
      ...
      for(int i = 0; i + 1 < bones; ++i) {
         m = mulYX(m, (double)joints[i][1], (double)joints[i][0]);
         double ox = (double)PIVOT[i + 1][0] / (double)16.0F * (double)scale;
         double oy = (double)PIVOT[i + 1][1] / (double)16.0F * (double)scale;
         double oz = (double)PIVOT[i + 1][2] / (double)16.0F * (double)scale;
   ...
   private static double[] mulYX(double[] a, double yaw, double pitch) {
      ...
      double[] r = new double[]{cy, sy * sx, sy * cx, (double)0.0F, cx, -sx, -sy, cy * sx, cy * cx};
   ...
   public static float[] aimAngles(Vec3 local) {
      ...
      return len < 1.0E-4 ? new float[]{0.0F, 0.0F} : new float[]{(float)(-Math.asin(Mth.clamp(local.y / len, (double)-1.0F, (double)1.0F))), (float)Math.atan2(local.x, local.z)};
   }
   ...
   static {
      BONES = BONE_LEN.length + 1;
      PIVOT = new float[][]{{0.0F, 0.0F, 0.0F}, {2.0F, 1.0F, 12.0F}, {1.0F, 3.0F, 8.0F}, {-1.0F, 3.0F, 16.0F}, {-1.0F, -3.0F, 18.1F}, {0.0F, 0.0F, 17.9F}, {-0.94F, -2.0014F, 10.0046F}, {1.54F, -0.9986F, 15.9954F}, {0.0F, 1.0F, 7.0F}, {0.4F, -1.0F, 10.0F}, {-1.5F, 0.5F, 7.0F}};
      double total = (double)0.0F;

      for(float len : BONE_LEN) {
         total += (double)len / (double)16.0F;
      }
