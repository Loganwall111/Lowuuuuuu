...
public class WitherStormWorldConfig extends SavedData {
   ...
   static {
      ...
      keyToggle("buildingDestruction", "The storm actively tears houses, village structures, and roofs apart into floating debris clusters", (c) -> (double)c.buildingDestruction, (c, v) -> c.buildingDestruction = (int)v);
      key("buildingTearRadius", "Radius around the storm scanned for constructed buildings to tear apart", 10.0, 96.0, true, (c) -> c.buildingTearRadius, (c, v) -> c.buildingTearRadius = v);
      key("buildingTearInterval", "Ticks between tearing chunks of buildings", 10.0, 300.0, true, (c) -> (double)c.buildingTearInterval, (c, v) -> c.buildingTearInterval = (int)v);
      key("buildingClusterSize", "Max block cluster radius from torn buildings", 1.0, 5.0, true, (c) -> (double)c.buildingClusterSize, (c, v) -> c.buildingClusterSize = (int)v);
      keyToggle("groundShakeOnSlam", "Tentacle slams cause an earthquake tremor and camera shake for nearby players", (c) -> (double)c.groundShakeOnSlam, (c, v) -> c.groundShakeOnSlam = (int)v);
      key("groundShakeRadius", "Radius of earthquake camera shake from tentacle slams", 20.0, 250.0, true, (c) -> c.groundShakeRadius, (c, v) -> c.groundShakeRadius = v);
      keyToggle("groundShockwaveParticles", "Tentacle slams emit radial shockwaves, explosion dust rings, and dragon breath", (c) -> (double)c.groundShockwaveParticles, (c, v) -> c.groundShockwaveParticles = (int)v);
      keyToggle("superCataclysmLightning", "Cataclysm Phase (5.8+) discharges violent purple lightning strikes into terrain", (c) -> (double)c.superCataclysmLightning, (c, v) -> c.superCataclysmLightning = (int)v);
      key("lightningDischargeInterval", "Ticks between cataclysm purple lightning strikes", 20.0, 400.0, true, (c) -> (double)c.lightningDischargeInterval, (c, v) -> c.lightningDischargeInterval = (int)v);
      ...
      key("tentacleThrowPower", "Catapult fling velocity when the tentacle throws a victim across the sky", 1.0, 6.0, false, (c) -> c.tentacleThrowPower, (c, v) -> c.tentacleThrowPower = v);
      key("tentacleChompDamage", "Damage inflicted when a head chomps and eats a snatched victim", 10.0, 500.0, false, (c) -> c.tentacleChompDamage, (c, v) -> c.tentacleChompDamage = v);
      key("tentacleSnatchRange", "Horizontal detection radius for tentacle swoops and grabs", 10.0, 80.0, true, (c) -> c.tentacleSnatchRange, (c, v) -> c.tentacleSnatchRange = v);
      keyToggle("tentacleTargetMobs", "Tentacles swoop at and snatch nearby animals, golems, and monsters in addition to players", (c) -> (double)c.tentacleTargetMobs, (c, v) -> c.tentacleTargetMobs = (int)v);
      key("tentacleEscapeHits", "Hits with a weapon required to sever a tentacle's grip and escape", 1.0, 15.0, true, (c) -> (double)c.tentacleEscapeHits, (c, v) -> c.tentacleEscapeHits = (int)v);
      key("debrisTornadoSpeed", "Swirling speed of debris and block clusters orbiting the storm", 0.1, 4.0, false, (c) -> c.debrisTornadoSpeed, (c, v) -> c.debrisTornadoSpeed = v);
      key("debrisDamageMultiplier", "Damage multiplier when flying block debris strikes players", 0.0, 5.0, false, (c) -> c.debrisDamageMultiplier, (c, v) -> c.debrisDamageMultiplier = v);
      key("bossHealthMultiplier", "Overall health multiplier for the Wither Storm and Devourer", 0.1, 10.0, false, (c) -> c.bossHealthMultiplier, (c, v) -> c.bossHealthMultiplier = v);
      key("bossAttackDamageMultiplier", "Damage multiplier for all storm attacks, skulls, beams, and slams", 0.1, 10.0, false, (c) -> c.bossAttackDamageMultiplier, (c, v) -> c.bossAttackDamageMultiplier = v);
      key("tractorBeamPullPower", "Pull force of tractor beams on caught entities", 0.1, 3.0, false, (c) -> c.tractorBeamPullPower, (c, v) -> c.tractorBeamPullPower = v);
      key("tractorBeamLiftSpeed", "Upward speed of tractor beams lifting entities to the maws", 0.1, 2.5, false, (c) -> c.tractorBeamLiftSpeed, (c, v) -> c.tractorBeamLiftSpeed = v);
