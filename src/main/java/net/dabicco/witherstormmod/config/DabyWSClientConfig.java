...
public class DabyWSClientConfig {
   ...
   static {
      ...
      key("trailerShadows", "Enable cinematic deep trailer shadow casting under the colossal storm body.", 0, 1, true, () -> trailerShadows ? 1 : 0, v -> trailerShadows = v >= 0.5);
      key("groundShakingTremors", "Camera shake and earthquake screen rumbles during tentacle slams, roars, and explosions.", 0, 1, true, () -> groundShakingTremors ? 1 : 0, v -> groundShakingTremors = v >= 0.5);
      key("screenTremorIntensity", "Earthquake screen rumble and camera tremor multiplier.", 0.1, 4.0, false, () -> screenTremorIntensity, v -> screenTremorIntensity = v);
      key("stormProximityVignette", "Atmospheric dark purple vignette encircling the screen edges when near the storm.", 0, 1, true, () -> stormProximityVignette ? 1 : 0, v -> stormProximityVignette = v >= 0.5);
      key("vignetteIntensity", "Dark storm vignette intensity multiplier.", 0.1, 3.0, false, () -> vignetteIntensity, v -> vignetteIntensity = v);
      key("sicknessVeinOverlay", "Creeping necrotic vein HUD overlay when afflicted with Wither Sickness.", 0, 1, true, () -> sicknessVeinOverlay ? 1 : 0, v -> sicknessVeinOverlay = v >= 0.5);
      key("sicknessVeinIntensity", "Strength and visibility of creeping necrotic veins.", 0.1, 3.0, false, () -> sicknessVeinIntensity, v -> sicknessVeinIntensity = v);
      key("chromaticGlitchStrength", "Screen glitch, chromatic distortion, and visual shockwaves during big roars.", 0.0, 3.0, false, () -> chromaticGlitchStrength, v -> chromaticGlitchStrength = v);
      key("storyModeBossbar", "Story Mode boss health bar with purple gem framing and phase runes.", 0, 1, true, () -> storyModeBossbar ? 1 : 0, v -> storyModeBossbar = v >= 0.5);
      key("storyModeTitleScreen", "Story Mode atmospheric title screen banner and quick-launch buttons.", 0, 1, true, () -> storyModeTitleScreen ? 1 : 0, v -> storyModeTitleScreen = v >= 0.5);
      key("customSkyboxes", "Dynamic Story Mode FabricSkyBoxes with day, night, and cataclysm skies.", 0, 1, true, () -> customSkyboxes ? 1 : 0, v -> customSkyboxes = v >= 0.5);
      key("cloudDeckLayer", "Minecraft Story Mode volumetric cloud deck shader overrides.", 0, 1, true, () -> cloudDeckLayer ? 1 : 0, v -> cloudDeckLayer = v >= 0.5);
      key("regionalBiomeFog", "Regional atmospheric biome fog transition system across 6 world regions.", 0, 1, true, () -> regionalBiomeFog ? 1 : 0, v -> regionalBiomeFog = v >= 0.5);
      key("purpleLightningSparks", "Ambient purple lightning electrical discharges around the storm crown.", 0, 1, true, () -> purpleLightningSparks ? 1 : 0, v -> purpleLightningSparks = v >= 0.5);
      key("debrisDustParticles", "Flying debris dust, block splinters, and smoke particle density.", 0.0, 5.0, false, () -> debrisDustParticles, v -> debrisDustParticles = v);
      key("headEyeGlow", "Colossal glowing eye beams and core eye lens flare glow.", 0, 1, true, () -> headEyeGlow ? 1 : 0, v -> headEyeGlow = v >= 0.5);
      key("volumetricFogDensity", "Volumetric fog, mist, and atmospheric haze density.", 0.0, 4.0, false, () -> volumetricFogDensity, v -> volumetricFogDensity = v);
      key("dynamicScreenShake", "Subtle footsteps ground tremor when the storm looms overhead.", 0, 1, true, () -> dynamicScreenShake ? 1 : 0, v -> dynamicScreenShake = v >= 0.5);
