...
public final class StormBloomProbe {
            ...
            if (reports < 6) {
               ...
               if (w > 0 && h > 0) {
                  ...
                  try {
                     ...
                     System.out.println("[dabywitherstormmod] bloom probe failed: " + String.valueOf(e));
                  }
   ...
   private static float halfToFloat(int bits) {
      ...
      float value;
      if (exp == 0) {
         value = (float)frac * (float)Math.pow((double)2.0F, (double)-24.0F);
      } else if (exp == 31) {
         value = frac == 0 ? Float.POSITIVE_INFINITY : Float.NaN;
      } else {
         value = (1.0F + (float)frac / 1024.0F) * (float)Math.pow((double)2.0F, (double)(exp - 15));
      }
   ...
   private static void report(GpuBuffer buffer, int w, int h, int fullW, int fullH) {
      try {
         ...
         try {
            ...
            ++reports;
            if (lit == 0) {
               System.out.println("[dabywitherstormmod] no lit pixels over the centre " + w + "x" + h + " of " + fullW + "x" + fullH + " (" + 100 * w / Math.max(1, fullW) + "% of width) -- if the storm is off to one side this crop can miss it entirely; centre the head and re-check before concluding the buffer is empty. The blur and the composite are irrelevant until this reports pixels.");
            } else {
               float peak = Math.max(halfToFloat(maxR), Math.max(halfToFloat(maxG), halfToFloat(maxB)));
               System.out.println("[dabywitherstormmod] bloom buffer OK: " + lit + "/" + pixels + " lit pixels in the centre " + w + "x" + h + ", peak channel " + String.format("%.2f", peak) + (peak > 1.0F ? " (HDR, good)" : " (NOT above 1.0 -- the source has no headroom, so the blur has nothing to spread)"));
            }
         ...
         System.out.println("[dabywitherstormmod] bloom probe read failed: " + String.valueOf(e));
      }
